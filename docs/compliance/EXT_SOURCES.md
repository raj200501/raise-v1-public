...
| `rfc_txt` | 79 `rfcNNNN.txt` files (rfc-editor.org) | Each RFC's Copyright Notice: copyright IETF Trust and the document authors, subject to BCP 78 and the IETF Trust Legal Provisions (https://trustee.ietf.org/license-info), which grant the right to reproduce and distribute RFCs; the older RFCs among the 79 (for example RFC 791, 793, 1034, 1035, 2616) predate BCP 78 and carry the earlier ISOC copyright statement permitting reproduction and distribution | `sources/rfc9110_copyright_notice_as_served_2026-09-09.txt` (one representative; the pre-2008 statements were read in the files and are not separately committed) |
