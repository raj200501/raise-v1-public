#!/usr/bin/env python3
"""Frozen reader for preregistration 0021 — real-content fit at 4096, rerun: does the encoder signal exist in
real files at all?
...
ARTIFACT = os.path.join(REPO, "artifacts", "pivot", "realfit_4096_rerun.json")
SCORES = os.path.join(REPO, "artifacts", "pivot", "realfit_4096_rerun_scores.json")
NOT_RUN = os.path.join(REPO, "artifacts", "pivot", "realfit_4096_rerun_not_run.json")
OUT = os.path.join(REPO, "artifacts", "pivot", "realfit_4096_rerun_verdict.json")
PREREG = "0021-realfit-4096-rerun"
...
PROTOCOL = {'seed': 20260825,
 ...
 'recipe_of': {'repro': 'incumbent',
               ...
               'builder_chunk_matched': 'model',
               'floor_majority': 'floor_majority',
               'floor_stratified': 'floor_stratified',
               'floor_feat1': 'floor_feat1',
               'floor_tree3': 'floor_tree3'},
 'repro_rows': 100000,
 'matched_rows': 26346,
 'chunk_matched_chunks': 1029,
 'floor_margin': 0.05,
 ...
 'fit_block_hashes': {'repro_sorted_sha256': '38de80c9bf49f2ea8be8e52545632a0904de4b57dc00256c165123efeed7596a',
                      ...
                      'chunk_matched_sorted_sha256': '735b257703ac2ca721514520ce75de478472c9c2122ec1ccd33dacc5b2efdcca',
                      'chunk_matched_idx_sha256': '6fa27a56443ec9f7e8b63cd24bac01fcb28b3065a8020a9375feaed26cea39a9',
 ...
 'block_source_chunks': {'repro_block': 37108,
                         'matched_block': 19413,
                         'chunk_matched_block': 1029,
                         'fit_block': 1029},
 'validation_split_chunk_contamination': {'fit_block': 1.0,
                                          'matched_block': 0.438,
                                          'chunk_matched_block': 1.0,
                                          'repro_block': 0.8977},
 ...
 'expected_rows_identical_to_a_scored_row': {'fit_block': 0,
                                             'repro_block': 8,
                                             'matched_block': 2,
...
RECIPES = {'incumbent': {'family': 'hgb',
 ...
 'model': {'family': 'hgb',
           ...
           'why': 'capacity plus regularisation plus a lower rate together: 800000 rows x 26 classes supports '
                  ...
                  '8 to 16 at 4096'},
 'floor_majority': {'id': 'B-majority',
                    'family': 'dummy',
                    'params': {'strategy': 'most_frequent'},
                    'scaled': False,
                    'val': 'frag',
                    'why': 'always predict the most frequent label of the fit block; '
                           "tools/trivial_baselines.py's `majority`"},
 'floor_stratified': {'id': 'B-stratified',
                      'family': 'dummy',
                      'params': {'strategy': 'stratified'},
                      'scaled': False,
                      'val': 'frag',
                      'why': "sample from the fit block's label prior, ignoring the input entirely; "
                             '`stratified`'},
 'floor_feat1': {'id': 'B-feat1',
                 'family': 'tree',
                 'params': {'max_depth': 1},
                 'scaled': False,
                 'val': 'frag',
                 'why': 'the single most informative feature, thresholded by a depth-1 tree; '
                        '`best_single_feat`'},
 'floor_tree3': {'id': 'B-tree3',
                 'family': 'tree',
                 'params': {'max_depth': 3},
                 'scaled': False,
                 'val': 'frag',
                 'why': 'a decision tree of depth 3 - a rule a person could write out by hand; `depth3_tree`'}}
RECIPES_SHA256 = "715e8f116bf12921de12bb8a4c3d16a6c8930276beaa8c7ccac5ddd0e3a5f147"
RECIPE_SHA256 = {'incumbent': '618e84840970bfe49e44fabdb0100895acd65ac4c042f0b402abe24c58386c9f',
 ...
 'floor_majority': '8476cde1058cff329232a522eac7bde3e32e844c3982b294fa4d309151489f82',
 'floor_stratified': 'c7ca7fa12989a757b958af3d0cedb9ed89487e6661870725827ca83dcb421596',
 'floor_feat1': '22bad8b51105e5c73094ccc9d154eba2ebe6276c03b34c3bf72b164c5b761339',
 'floor_tree3': '61971b09304412a6803846358a343ea594ee78cff4f671745703c15379cef159'}
...
PARTITION = {'seed': 20260825,
 ...
 'matched_sorted_sha256': 'f08557aee0c9dbdedf90d0d670a73b0277d1572a5e9e3cb7f07e9128e852ec6b',
 'chunk_matched_rows': 20817,
 'chunk_matched_idx_sha256': '6fa27a56443ec9f7e8b63cd24bac01fcb28b3065a8020a9375feaed26cea39a9',
 'chunk_matched_sorted_sha256': '735b257703ac2ca721514520ce75de478472c9c2122ec1ccd33dacc5b2efdcca',
 'chunk_matched_block_chunks': 1029,
 'repro_block_chunks': 37108,
 'matched_block_chunks': 19413,
...
NULL_TOLERANCE = 0.01
ORDER = ["repro", "null", "real_model", "real_incumbent", "real_logistic", "floor_majority", "floor_stratified", "floor_feat1", "floor_tree3", "builder_matched", "builder_chunk_matched"]
RECIPE_OF = {'repro': 'incumbent',
 'null': 'model',
 'real_model': 'model',
 'real_incumbent': 'incumbent',
 ...
 'floor_feat1': 'floor_feat1',
 'floor_tree3': 'floor_tree3'}
# A corpus-build statistic the run does not measure: published in the verdict, never compared against the artifact,
# because the runner does not bank it and a reader that demands it would void an honest run.
FIT_CEILINGS = {'c_src': 0.9712, 'pe_bin': 0.9069, 'py_src': 0.9765, 'rfc_txt': 0.9778, 'rst_doc': 0.9567}
# Every key the normal path writes, so the exception path can write the same shape with None values.
RESULT_KEYS = ['bar_applied',
 ...
 'ext_per_family',
 'ext_real_correct',
 'ext_real_correct_model',
 'ext_real_top1',
 'ext_real_top1_model',
 'ext_rows_per_family',
 'ext_subsets',
 'ext_top1',
 'fit_ceiling_distinct_fragments',
 'informational_only',
...
STAGE_OF = dict({"repro": "pool_rung", "null": "fit_null", "real_model": "realfit", "real_incumbent": "realfit",
                 "real_logistic": "realfit", "builder_matched": "pool_matched",
                 "builder_chunk_matched": "pool_chunk_matched"},
                **{n: "realfit_floor" for n in FLOOR_ROLES})
ROWS_OF = dict({"repro": ("repro_rows", "repro_idx_sha256", "repro_sorted_sha256"),
                ...
                "builder_chunk_matched": ("chunk_matched_rows", "chunk_matched_idx_sha256",
                                          "chunk_matched_sorted_sha256")},
               **{n: ("fit_rows", "fit_idx_sha256", "fit_sorted_sha256") for n in FLOOR_ROLES})
MAX_ITER = {'incumbent': 200,
 'logistic_l3': 1000,
 'model': 250,
 'floor_majority': 0,
 'floor_stratified': 0,
...
# The banked reading of the SAME rows under a builder-fitted M4 at 800000 rows, for the comparison flag only.
# It comes from 0019, the re-read: 0018's own frozen reader emitted VOID and never produced these numbers.
OOB_RUN = "0018-oob-4096"
OOB_PREREG = "0019-oob-4096-reread"
...
def main() -> int:
    ...
    if os.path.exists(NOT_RUN):
        try:
            ...
            print(f"READER 0021: NOT RUN — {nr.get('reason')!r} (stage {nr.get('stage')}, filed {nr.get('utc')}, "
                  f"{nr.get('n_checkpoints')} checkpoints). A NOT RUN is not a verdict.", file=sys.stderr)
        except Exception:  # noqa: BLE001
            print("READER 0021: NOT RUN file present but unreadable.", file=sys.stderr)
        return 2
    if not os.path.exists(ARTIFACT):
        print(f"READER 0021: artifact absent: {os.path.relpath(ARTIFACT, REPO)}\n  Absence is not a pass. No verdict emitted.",
              file=sys.stderr)
        return 2
    try:
        d = json.load(open(ARTIFACT, encoding="utf-8"))
    except Exception as e:  # noqa: BLE001
        print(f"READER 0021: artifact malformed ({type(e).__name__}: {e}).", file=sys.stderr)
        return 2
    try:
        return read(d)
    except Exception as e:  # noqa: BLE001
        ...
        result = dict({k: None for k in RESULT_KEYS},
                      schema="raise-v1/realfit_4096_rerun_verdict/1", preregistration=PREREG,
        ...
        _write(result)
        print(f"READER 0021 — VOID: artifact of unexpected shape ({type(e).__name__}: {e})\n  wrote {os.path.relpath(OUT, REPO)}")
        return 0
...
def read(d) -> int:
    ...
    # The counts that bear on the CLAUSE must be zero: the fit corpus is the arm the verdict is read from and must
    # share nothing with anything scored, at any level.
    for k in ("pool_chunks_shared_with_ext", "eval_chunks_shared_with_ext", "fit_chunks_shared_with_ext",
              ...
              "fit_rows_identical_to_a_scored_row"):
        if part.get(k) != 0:
            void.append(f"leakage: partition.{k}={part.get(k)!r}, must be the measured 0")
    # The BUILDER blocks' counts must equal the sealed expectations EXACTLY, and zero is not the right answer for
    # them. 0020 required zero of all four and refused at launch on repro 8 and matched 2, which are not leaks: the
    # corpus builder emitted nine pairs of source chunks that carve to identical feature rows, one of which straddles
    # 0003's grouped split. A count that grows means new content crossed; one that shrinks means these are not the
    # sealed blocks. Both are VOID.
    for k, want in EXPECTED_COLLISIONS.items():
        got = part.get({"fit_block": "fit_rows_identical_to_a_scored_row",
                        "repro_block": "repro_rows_identical_to_a_scored_row",
                        "matched_block": "matched_rows_identical_to_a_scored_row",
                        "chunk_matched_block": "chunk_matched_rows_identical_to_a_scored_row"}[k])
        if got != want:
            void.append(f"leakage: the {k}'s rows byte-identical to a scored row is {got!r}, not the "
    ...
    if not _ints(eval_ids, n_eval, None) or min(eval_ids) < CORPUS["chunk_id_min"] or max(eval_ids) > CORPUS["chunk_id_max"] \
        ...
        # Range, distinct count and family mix are all permutation-invariant, and fam_e - which drives every record's
        # per_family and every builder_eval_per_family reading - is derived from this array position by position. The
        # extension side was bound by a digest and this one was not (0020 pre-freeze review, reader lens, finding 7).
        try:
        ...
        if hashlib.sha256(_eb).hexdigest() != PARTITION["eval_g_sha256"] or list(eval_ids) != _expected:
            void.append("scores: eval_chunk_ids are not the sealed evaluation chunk ids expanded by their sealed "
                        "row counts, or do not hash to the sealed evaluation g array")
        else:
    ...
    def check_record(name):
        ...
        if fam_e is not None:
            ...
            # top1_non_gutenberg was required to be finite and then taken on trust. run_recipe_search.score() labels
            # every extension row "ext:<family>", so all N_EXT of them count as non-gutenberg (0020 pre-freeze review,
            # reader lens, finding 9).
            tng = mean4([pe[i] for i in range(n_eval) if fam_e[i] != "gutenberg"] + list(pe[n_eval:]))
            if num(rec.get("top1_non_gutenberg")) != tng:
                void.append(f"fit: {name} record top1_non_gutenberg {rec.get('top1_non_gutenberg')!r} is not its "
                            f"vector's non-gutenberg mean {tng!r}")

    if set(fits) != set(ORDER):
        extra = sorted(set(fits) - set(ORDER)); absent = sorted(set(ORDER) - set(fits))
        void.append(f"fit: the artifact banks arms outside the sealed order (extra {extra}, missing {absent}) - "
    ...
    # ---- the floor, MEASURED on these rows by the four trivial-baseline arms (OPERATING_RULES 4a), then the clause
    floor_real = floor_by_family = floor_arm = None
    need_real = MIN_CORRECT_REAL
    if not void:
        floor_real = max(rec_reads[n]["ext_real_top1"] for n in FLOOR_ROLES)
        floor_arm = sorted(n for n in FLOOR_ROLES if rec_reads[n]["ext_real_top1"] == floor_real)[0]
        floor_by_family = {f: max(rec_reads[n]["ext_per_family"][f] for n in FLOOR_ROLES) for f in EXT_FAMILIES}
        need_real = int(math.ceil(N_REAL * (floor_real + BAR) - 1e-9))

    fails: list[str] = []
    mreal = rec_reads.get("real_model", {}).get("ext_real_correct") if not void else None
    if not void and mreal < need_real:
        fails.append(f"clause: real_model's correct real-family rows {mreal} of {N_REAL} "
                     f"({rec_reads['real_model']['ext_real_top1']}) is below {need_real} "
                     f"(floor {floor_real} from {floor_arm} + {BAR})")

    flags = None
    if not void:
        rm = rec_reads["real_model"]; bm = rec_reads["builder_matched"]
        bcm = rec_reads["builder_chunk_matched"]
        ri = rec_reads["real_incumbent"]; rl = rec_reads["real_logistic"]
        need_fam = {f: int(math.ceil(EXT["rows_per_family"][f] * (floor_by_family[f] + BAR) - 1e-9))
                    for f in EXT_FAMILIES}
        fam_clear = [f for f in REAL_FAMILIES if rm["ext_per_family_correct"][f] >= need_fam[f]]
        flags = {"floor_real_top1": floor_real, "floor_arm": floor_arm, "floor_by_family": floor_by_family,
                 "floor_by_arm": {n: rec_reads[n]["ext_real_top1"] for n in FLOOR_ROLES},
                 "need_real_over_floor": need_real, "need_per_family_over_floor": need_fam,
                 "families_clearing": len(fam_clear), "families_clearing_names": fam_clear,
                 "n_real_families": len(REAL_FAMILIES),
                 "min_correct_real_chance_based_0018_comparable": MIN_CORRECT_REAL,
                 "real_model_reaches_0018_chance_bar": mreal >= MIN_CORRECT_REAL,
                 "n_real_rows": N_REAL,
                 "builder_matched_reaches_bar_real": bm["ext_real_correct"] >= need_real,
                 "builder_chunk_matched_reaches_bar_real": bcm["ext_real_correct"] >= need_real,
                 "real_model_leads_builder_chunk_matched_real_rows": rm["ext_real_correct"] - bcm["ext_real_correct"],
                 "block_source_chunks": BLOCK_SOURCE_CHUNKS,
                 "validation_split_chunk_contamination": VAL_CONTAM,
                 "a_sister_recipe_clears_and_real_model_does_not":
                     (ri["ext_real_correct"] >= need_real or rl["ext_real_correct"] >= need_real)
                     and mreal < need_real,
                 "real_incumbent_reaches_bar_real": ri["ext_real_correct"] >= need_real,
                 "real_logistic_reaches_bar_real": rl["ext_real_correct"] >= need_real,
                 "real_model_leads_builder_matched_real_rows": rm["ext_real_correct"] - bm["ext_real_correct"],
                 "real_model_leads_0018_run_builder_fit_real_rows": rm["ext_real_correct"] - OOB_MODEL_REAL_CORRECT,
                 "oob_0018_run_read_by_0019_builder_fit_real_correct": OOB_MODEL_REAL_CORRECT,
                 "oob_comparison_run": OOB_RUN, "oob_comparison_reader": OOB_PREREG,
                 "oob_0018_run_read_by_0019_builder_fit_real_top1": OOB_MODEL_REAL_TOP1,
                 "synthetic_reaches_bar": rm["ext_synthetic_correct"] >= need(N_EXT - N_REAL),
                 "synthetic_min_correct": need(N_EXT - N_REAL),
                 "per_family_min_correct_chance_based": {f: need(EXT["rows_per_family"][f]) for f in EXT_FAMILIES},
                 "families_where_real_model_reaches_chance_bar": [f for f in EXT_FAMILIES
    ...
    if void:
        ...
        verdict, meaning = "REAL_FIT_FAILS", (
            f"At 4096 bytes, 0014's searched model fitted on {FIT['n_rows']} rows from {FIT['n_chunks']} plaintexts of real "
            f"content does not beat the best trivial baseline on the same rows by {BAR} on held-out chunks of the same five "
            f"pinned real files: {mreal} of {N_REAL} rows correct ({rec_reads['real_model']['ext_real_top1']}) against "
            f"{need_real} needed, where the floor is {floor_real} measured by {floor_arm} fitted on the same fit block. "
            f"{flags['families_clearing']} of {len(REAL_FAMILIES)} real families clear their own floor + {BAR} "
            f"({', '.join(flags['families_clearing_names']) or 'none'}). A bound on this recipe at this budget on these "
            f"five file kinds, read IN DISTRIBUTION — the fit and scored chunks come from the same files and share no byte, "
            f"no chunk id and no source-chunk hash — and not a statement about real content in general. The two "
            f"builder-matched arms (same row budget; same plaintext budget), the per-family readings and the two other "
            f"recipes banked beside it say where the reading comes from. On 0018's chance-based bar the same count reads "
            f"{mreal} against {MIN_CORRECT_REAL}, which is how it compares with 0018's {OOB_MODEL_REAL_CORRECT}. Nothing "
            f"here revises 0003, 0014, 0015, 0016, 0017, 0018 or 0019, and nothing here establishes a buyer.")
    else:
        verdict, meaning = "REAL_FIT_CLEARS", (
            f"At 4096 bytes, 0014's searched model fitted on {FIT['n_rows']} rows from {FIT['n_chunks']} plaintexts of real "
            f"content beats the best trivial baseline on the same rows by {BAR} or more on held-out chunks of the same five "
            f"pinned real files: {mreal} of {N_REAL} rows correct ({rec_reads['real_model']['ext_real_top1']}) against "
            f"{need_real} needed, where the floor is {floor_real} measured by {floor_arm} fitted on the same fit block. "
            f"{flags['families_clearing']} of {len(REAL_FAMILIES)} real families clear their own floor + {BAR} "
            f"({', '.join(flags['families_clearing_names']) or 'none'}), which is what says whether this is a statement "
            f"about real content or about one family. This is an IN-DISTRIBUTION reading on real content, not a transfer "
            f"reading: the fit and scored chunks are cut from the same five pinned blobs and share no byte, no chunk and no "
            f"source chunk, but they are the same files, so this says the signal is reachable when the model has seen "
            f"content of that kind — not that it would be reachable on a real file kind never seen (0018 read that, and it "
            f"failed). On 0018's chance-based bar the same count reads {mreal} against {MIN_CORRECT_REAL}, against 0018's "
            f"{OOB_MODEL_REAL_CORRECT}. Nothing here revises 0003, 0014, 0015, 0016, 0017, 0018 or 0019, and nothing here "
            f"establishes a buyer.")

    result = {
        "schema": "raise-v1/realfit_4096_rerun_verdict/1",
        ...
        "bar_applied": {"floor_real_top1": floor_real, "floor_arm": floor_arm, "bar": BAR,
                        "min_correct_real_over_floor": need_real if not void else None,
                        "chance": CHANCE, "must_reach_chance_based": MUST_REACH,
                        "min_correct_real_chance_based": MIN_CORRECT_REAL,
                        "families_clearing": (flags or {}).get("families_clearing"),
                        "n_real_rows": N_REAL, "n_ext_rows": N_EXT, "fit_rows": FIT["n_rows"],
                        "fit_source_chunks": FIT["n_chunks"], "flags": flags},
        ...
        "fit_ceiling_distinct_fragments": FIT_CEILINGS,
    ...
    print("READER 0021 — real-content fit at 4096 (in distribution, on the same pinned files)")
