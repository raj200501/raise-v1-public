#!/usr/bin/env python3
"""Frozen reader for preregistration 0022 — symmetric recipe search on REAL content at 4096.

Written and committed BEFORE any fit was made under this preregistration. Every literal below was generated
from prereg/0022-realsearch-4096.json or read from a banked artifact; none was typed here.

WHY THIS EXISTS. 0021 read REAL_FIT_FAILS: 0014's searched model M4, fitted UNSEARCHED on 26346 rows of real
content, identifies the encoder on the 38452 sealed real-family rows at 0.1193, a depth-3 tree on the same rows
at 0.0929 and 0014's standardised logistic at 0.1179. 0021's scope deferred one question - what a recipe searched
on real content would do. This run answers it the way 0014 answered the same question on builder content:
symmetrically. Every head with a hyperparameter gets 0014's roster verbatim, selected by 0014's rule on a chunk-rule
holdout cut inside the real fit block; each head's winner is fitted once on the whole fit block and scored once on
0018's scoring set. Four reproduction arms tie the run to 0021's banked readings.

This reader takes the runner's artifact (tools/pivot/run_realsearch.py) and checks, clause by clause: the sealed
corpora (array hashes for the builder cache, the evaluation corpus and the fit corpus); the sealed split, the sealed
fit block, holdout, fit pool and stage (hash for hash); that nothing scored was fitted (the runner's measured chunk,
source-chunk and row-identity counts must all be 0; the holdout and the fit pool share nothing; the selection file
gathered fit-corpus rows only); that the search ran as enumerated and the banked winners are the rule's; that every
confirmatory fit is the selected recipe on the sealed block with the sealed seed, scored exactly once, in the sealed
order, with its fingerprint recomputed here; that each record's own scores equal what its per-example vector gives;
that each reproduction arm lands within 0.005 of 0021's banked value; that the null control sits at chance on both
row sets; that every banked reading and every banked bar equals what this reader recomputes. Then two clauses: the
model's count of correct real-family rows against the best searched baseline of the FROZEN set plus 0.05, and
against the best of the EXPANDED set plus 0.05, each baseline floored at 0021's reading of it.
REAL_RECIPE_CLEARS or REAL_RECIPE_FAILS, published at the same size; VOID on any validity failure.

NOT A TRANSFER READING. The fit block and the scored chunks are cut from the same five pinned blobs; they share no
byte, no chunk index, no chunk id and no source-chunk hash, and the runner measures that rather than assuming it.
Every verdict string this reader emits says so.
...
ARTIFACT = os.path.join(REPO, "artifacts", "pivot", "realsearch_4096.json")
SELECTION = os.path.join(REPO, "artifacts", "pivot", "realsearch_4096_selection.json")
SCORES = os.path.join(REPO, "artifacts", "pivot", "realsearch_4096_scores.json")
NOT_RUN = os.path.join(REPO, "artifacts", "pivot", "realsearch_4096_not_run.json")
OUT = os.path.join(REPO, "artifacts", "pivot", "realsearch_4096_verdict.json")
PREREG = "0022-realsearch-4096"
...
# the builder cache: 0003's corpus A, identified by the sha256 of y, g and (since 0021) X
...
# the sealed protocol, verbatim; the banked roster.protocol must equal it field by field
PROTOCOL = {'seed': 20260825,
 ...
 'incumbent_id': 'M1',
 'model_reference_id': 'M4',
 'logistic_reference_id': 'L3',
 'tree_reference_id': 'D1',
 'logistic_head_id': 'logistic',
 'model_head_id': 'model',
 'confirm_order': ['repro_m1',
                   'repro_m4',
                   'repro_l3',
                   'repro_d1',
                   'null',
 ...
 'expanded_heads': ['majority', 'stratified', 'depth3_tree', 'logistic', 'best_single_feat', 'deep_tree'],
 'floors': {'majority': 0.0387,
            'stratified': 0.0382,
            'depth3_tree': 0.0929,
            'logistic': 0.1179,
            'best_single_feat': 0.0549,
            'deep_tree': 0.0},
 'floors_source': "0021's banked readings on the same 38452 rows under the same recipes fitted on the same fit "
                  'block (artifacts/pivot/realfit_4096_rerun.json ext_real_top1: floor_majority, '
                  'floor_stratified, floor_tree3 = D1, real_logistic = L3, floor_feat1 = U3); deep_tree has no '
                  '0021 reading and is floored at 0.0, so that head can raise the expanded bar and never lower '
                  'it',
 'reference_top1': {'repro_m1': 0.1012, 'repro_m4': 0.1193, 'repro_l3': 0.1179, 'repro_d1': 0.0929},
 'reference_source': 'artifacts/pivot/realfit_4096_rerun.json ext_real_top1: real_incumbent (M1), real_model '
                     "(M4), real_logistic (L3), floor_tree3 (D1), each fitted on this fit block in 0021's "
                     'sealed order and scored on this scoring set; a searched head that selects its reference '
                     'recipe reproduces it with its own confirmatory fit',
 'reproduction_tolerance': 0.005,
 'null_tolerance': 0.01,
 'null_rows': 'all',
 'null_y_shuffled_sha256_expected': '23b5db271abb80fa2d16e1a007c9305380d3530de8b6a1f4e9773af71e1fc86e',
 'null_note': "the null control permutes the whole fit block's labels with the split's own generator, exactly "
              "as 0021 did at the same generator state, so the permuted label array is 0021's and its hash is "
              'sealed here',
 ...
 'fit_block_hashes': {'fit_idx_sha256': '2b5fbaf627f1073bdeb51499c213be18837185414607b314b19f0efb6f5f5469',
                      ...
                      'holdout_idx_sha256': '9e6170b723f343ed3773b1b8a3042e45da92d4c26260245cced82c9a2b0ce5a6',
                      'holdout_sorted_sha256': '6bad617a876e5bd946a29b8ec7ff6fb7b89a4139ea7a31489c28281cc011135e',
                      'holdout_chunks_sha256': 'adf17e6238047e514c96c80218affe38fecac24085d144f1bdbfbfb8c8ff8c51',
                      'fitpool_idx_sha256': '6b0ddd65567d62e9a2619e8a8cf476e18f89c1deb3ea71142ed124286843d5d7',
                      'fitpool_sorted_sha256': '2f052b240f2b5cbe1a9dbae2d1a74926b1a67644735bef2bc210c294ca84389e',
                      'fitpool_chunks_sha256': 'ad21e9416c3cc835b7a601106db5b7936eccc3b93bdcd7f5d34a2a52fd231291',
                      'stage_idx_sha256': ['6b0ddd65567d62e9a2619e8a8cf476e18f89c1deb3ea71142ed124286843d5d7'],
                      'stage_sorted_sha256': ['2f052b240f2b5cbe1a9dbae2d1a74926b1a67644735bef2bc210c294ca84389e']},
 ...
 'bar': {'margin': 0.05}}
PROTOCOL_SHA256 = "2c512e84e0d39b472be82b427839c557e153c810a7e48a07d27dd25dd18c244b"
ROSTER_SHA256 = "726f8c9387a2ca08626e5c317bfac96fec347b1598c2d6b413279a58c80185fc"
# 0014's roster heads, verbatim: ids, families, params, scaled, val and each candidate's why
HEADS = [{'id': 'model',
  'side': 'model',
  'candidates': [{'id': 'M1',
                  'family': 'hgb',
                 ...
                 {'id': 'M2',
                  ...
                  'params': {'learning_rate': 0.15,
                             ...
                             'max_iter': 400,
                             'n_iter_no_change': 20},
                  'why': 'horizon only: was the incumbent stopped early by patience 10 on a 26-class '
                         'validation loss that moves by thousandths? (at 2048 it fired after 22 iterations at '
                         '100000 rows and 38 at 500000)'},
                 {'id': 'M3',
                  'family': 'hgb',
                  'val': 'frag',
                  'params': {'learning_rate': 0.05,
                             ...
                             'max_iter': 300,
                             'n_iter_no_change': 20},
                  'why': 'lower rate, longer horizon: the standard boosting trade; the cap of 300 bounds the '
                         'worst-case cost'},
                 {'id': 'M4',
                  'family': 'hgb',
                  'val': 'frag',
                  'params': {'learning_rate': 0.05,
                             ...
                             'max_iter': 250,
                             'n_iter_no_change': 20},
                  'why': 'capacity plus regularisation plus a lower rate together: 800000 rows x 26 classes '
                         'supports bigger trees than 63 leaves, and the deep-tree baseline gained 0.1459 -> '
                         '0.1812 from depth 8 to 16 at 4096'},
                 {'id': 'M5',
                  'family': 'hgb',
                  'val': 'frag',
                  'params': {'learning_rate': 0.1,
                             ...
                             'max_iter': 150,
                             'n_iter_no_change': 15},
                  'why': 'the wide-tree end with the leaf-size regulariser that matters for 26-way splits on '
                         'noisy histogram columns; run_study.py records that unregularised capacity collapsed '
                         'the 100k rung once'},
                 {'id': 'M6',
                  'family': 'hgb',
                  'val': 'frag',
                  'params': {'learning_rate': 0.1,
                             ...
                             'max_iter': 400,
                             'n_iter_no_change': 20},
                  'why': 'per-split column subsampling: feature_ablation.json banks that the 555 alignment '
                         'columns carry little, i.e. about half the 1108-column vector is near-redundant, and '
                         'subsampling is the cheap decorrelator (new in sklearn 1.4, unavailable to the 0003 '
                         'pilot)'},
                 {'id': 'M7',
                  'family': 'hgb',
                  'val': 'frag',
                  'params': {'learning_rate': 0.15,
                             ...
                             'max_iter': 400,
                             'n_iter_no_change': 20},
                  'why': 'the low-variance end: small trees with heavy L2; at chance 0.038 and top-1 0.17 the '
                         'task is signal-starved and a lower-variance learner can beat a higher-capacity one '
                         'at the same data'},
                 {'id': 'M8',
                  'family': 'hgb',
                  'val': 'group',
                  'params': {'learning_rate': 0.15,
                             ...
                             'max_iter': 200,
                             'n_iter_no_change': 10},
                  'why': 'the incumbent with a CHUNK-GROUPED early-stopping split instead of the inherited '
                         'fragment-level one, which shares 18614 chunks with its fitting rows at 500000 rows '
                         'in 0012 (measured 2026-09-03): isolates whether the content-leaking validation split '
                         'stops the incumbent at the wrong iteration'}]},
 {'id': 'logistic',
  'side': 'frozen',
  'candidates': [{'id': 'L1',
                  'family': 'logistic',
                  ...
                  'why': "0003's logistic verbatim (LogisticRegression(max_iter=400), lbfgs, C 1.0, raw "
                         'features): the frozen-set bar at 0.1392; its confirmatory reproduction within 0.005 '
                         'is a validity clause'},
                 {'id': 'L2',
                  'family': 'logistic',
                  'params': {'max_iter': 1500},
                  'why': 'the 0003 recipe allowed to run to 1500 lbfgs iterations on raw features: tests '
                         "whether the 400-iteration cap alone held the frozen bar down (0003's run hit the "
                         'cap)'},
                 {'id': 'L3',
                  'family': 'logistic',
                  'scaled': True,
                 ...
                 {'id': 'L4',
                  ...
                  'why': 'standardised, stronger regularisation'},
                 {'id': 'L5',
                  'family': 'logistic',
                  'scaled': True,
                  'params': {'max_iter': 1000, 'C': 0.01},
                  'why': 'standardised, the strongest regularisation on the list, so the C axis is bracketed '
                         'below'},
                 {'id': 'L6',
                  'family': 'logistic',
                  'scaled': True,
                  'params': {'max_iter': 1000, 'C': 10.0},
                  'why': 'standardised, weak regularisation, so the C axis is bracketed above'},
                 {'id': 'L7',
                  'family': 'logistic',
                  'scaled': True,
                  'params': {'max_iter': 1500, 'C': 1.0},
                  'why': 'L3 with a longer iteration budget: convergence at 1500 against 1000 on the '
                         'standardised problem'},
                 {'id': 'L8',
                  'family': 'logistic',
                  'scaled': True,
                  'params': {'max_iter': 1500, 'C': 0.1},
                  'why': 'L4 with a longer iteration budget; max_iter is capped at 1500 across the head so the '
                         'worst-case logistic fit is bounded'}]},
 {'id': 'depth3_tree',
  'side': 'frozen',
  'candidates': [{'id': 'D1',
                  'family': 'tree',
                 ...
                 {'id': 'D2',
                  ...
                  'why': "depth is the head's definition and is not searched; criterion x min_samples_leaf is "
                         'the whole knob space at depth 3, searched so that every frozen head with a '
                         'hyperparameter gets the same treatment'},
                 {'id': 'D3',
                  'family': 'tree',
                 ...
                 {'id': 'D5',
                  ...
                  'why': 'as D2, entropy criterion'},
                 {'id': 'D6',
                  'family': 'tree',
                  'params': {'max_depth': 3, 'criterion': 'entropy', 'min_samples_leaf': 5},
                  'why': 'as D2, entropy criterion'},
                 {'id': 'D7',
                  'family': 'tree',
                  'params': {'max_depth': 3, 'criterion': 'entropy', 'min_samples_leaf': 20},
                  'why': 'as D2, entropy criterion'},
                 {'id': 'D8',
                  'family': 'tree',
                  'params': {'max_depth': 3, 'criterion': 'entropy', 'min_samples_leaf': 100},
                  'why': 'as D2, entropy criterion'}]},
 {'id': 'deep_tree',
  'side': 'expanded',
  'candidates': [{'id': 'T1',
                  'family': 'tree',
                  'params': {'max_depth': 8},
                  'why': "0003's depth8_tree verbatim (0.1459 banked); the deep_tree head subsumes 0003's "
                         'depth8_tree and depth16_tree heads and carries the floor max(0.1459, 0.1812) = '
                         '0.1812 (non-gutenberg 0.1891)'},
                 {'id': 'T2',
                  'family': 'tree',
                  'params': {'max_depth': 16},
                  'why': "0003's depth16_tree verbatim: the binding expanded-set baseline at 0.1812 (0.1891 "
                         'non-gutenberg)'},
                 {'id': 'T3',
                  'family': 'tree',
                  'params': {'max_depth': 16, 'min_samples_leaf': 20},
                  'why': 'depth 16 with leaf-size regularisation: the first-order fix for an unpruned depth-16 '
                         'tree on 800000 rows'},
                 {'id': 'T4',
                  'family': 'tree',
                  'params': {'max_depth': 24},
                  'why': 'the next step on the axis that banked 0.0969 -> 0.1459 -> 0.1812 from depth 3 to 8 '
                         'to 16'},
                 {'id': 'T5',
                  'family': 'tree',
                  'params': {'max_depth': 24, 'min_samples_leaf': 20},
                  'why': 'depth 24, regularised'},
                 {'id': 'T6',
                  'family': 'tree',
                  'params': {'max_depth': None},
                  'why': 'unlimited depth, unpruned: the maximal single tree'},
                 {'id': 'T7',
                  'family': 'tree',
                  'params': {'max_depth': None, 'min_samples_leaf': 5},
                  'why': 'unlimited depth, light leaf regularisation'},
                 {'id': 'T8',
                  'family': 'tree',
                  'params': {'max_depth': None, 'min_samples_leaf': 20},
                  'why': 'unlimited depth with min_samples_leaf 20: plausibly the strongest single tree '
                         'sklearn can build here, and the candidate most likely to RAISE the expanded bar'}]},
 {'id': 'majority',
  'side': 'frozen',
  'candidates': [{'id': 'U1',
                  'family': 'dummy',
 ...
 {'id': 'stratified',
  ...
  'candidates': [{'id': 'U2',
                  ...
                  'why': 'no hyperparameter; confirmatory fit only (0.0387 banked)'}]},
 {'id': 'best_single_feat',
  'side': 'expanded',
  'candidates': [{'id': 'U3',
                  'family': 'tree',
                  'params': {'max_depth': 1},
                  'why': 'a depth-1 tree has no meaningful knob; confirmatory fit only (0.0751 banked; '
                         'non-gutenberg 0.0769)'}]}]
FROZEN_HEADS = ["majority", "stratified", "depth3_tree", "logistic"]
EXPANDED_HEADS = ["majority", "stratified", "depth3_tree", "logistic", "best_single_feat", "deep_tree"]
MODEL_HEAD = "model"
# reproduction arm -> (head, roster id of 0021's recipe); aliased to the head's own confirmatory fit when selected
REPRO_ARMS = {'repro_m1': ['model', 'M1'],
 'repro_m4': ['model', 'M4'],
 'repro_l3': ['logistic', 'L3'],
 'repro_d1': ['depth3_tree', 'D1']}
CONFIRM_ORDER = ["repro_m1", "repro_m4", "repro_l3", "repro_d1", "null", "majority", "stratified", "depth3_tree", "logistic", "best_single_feat", "deep_tree", "model"]
# the partition constants the runner banks and this reader requires, hash for hash; every key is one the runner writes
PARTITION = {'seed': 20260825,
 ...
 'holdout_rule': 'fit-block chunk_id % 5 == 0',
 'n_holdout_rows': 5291,
 'n_holdout_chunks': 206,
 'holdout_idx_sha256': '9e6170b723f343ed3773b1b8a3042e45da92d4c26260245cced82c9a2b0ce5a6',
 'holdout_sorted_sha256': '6bad617a876e5bd946a29b8ec7ff6fb7b89a4139ea7a31489c28281cc011135e',
 'holdout_chunks_sha256': 'adf17e6238047e514c96c80218affe38fecac24085d144f1bdbfbfb8c8ff8c51',
 'holdout_rows_per_family': {'c_src': 130, 'pe_bin': 1014, 'py_src': 3471, 'rfc_txt': 468, 'rst_doc': 208},
 'holdout_chunks_per_family': {'c_src': 5, 'pe_bin': 40, 'py_src': 135, 'rfc_txt': 18, 'rst_doc': 8},
 'n_fitpool_rows': 21055,
 'n_fitpool_chunks': 823,
 'fitpool_idx_sha256': '6b0ddd65567d62e9a2619e8a8cf476e18f89c1deb3ea71142ed124286843d5d7',
 'fitpool_sorted_sha256': '2f052b240f2b5cbe1a9dbae2d1a74926b1a67644735bef2bc210c294ca84389e',
 'fitpool_chunks_sha256': 'ad21e9416c3cc835b7a601106db5b7936eccc3b93bdcd7f5d34a2a52fd231291',
 'fitpool_rows_per_family': {'c_src': 494, 'pe_bin': 4131, 'py_src': 13726, 'rfc_txt': 1872, 'rst_doc': 832},
 'stage_rows': [21055],
 'stage_keep': [1],
 'stage_chunks': [823],
 'stage_idx_sha256': ['6b0ddd65567d62e9a2619e8a8cf476e18f89c1deb3ea71142ed124286843d5d7'],
...
SEL_PARTITION = {k: v for k, v in PARTITION.items() if k != "eval_y_sha256"}   # labels are banked only where scored
...
FLOORS = {'majority': 0.0387,
 ...
 'logistic': 0.1179,
 'best_single_feat': 0.0549,
 'deep_tree': 0.0}                    # 0021's banked readings of each baseline head's reference recipe on these rows
REFERENCE = {'repro_m1': 0.1012, 'repro_m4': 0.1193, 'repro_l3': 0.1179, 'repro_d1': 0.0929}              # 0021's banked readings the four reproduction arms must reproduce
REPRO_TOLERANCE = 0.005
NULL_TOLERANCE = 0.01
NULL_SHA_EXPECTED = "23b5db271abb80fa2d16e1a007c9305380d3530de8b6a1f4e9773af71e1fc86e"     # 0021's permuted label array, hash for hash
...
MAX_ITER = {'M1': 200,
 ...
 'U1': 0,
 'U2': 0,
 'U3': 0}                # by roster id: the iteration cap of hgb and logistic candidates, 0 otherwise
RESULT_KEYS = ['aliased_reproductions',
 'bars_applied',
 'builder_eval_top1',
 'establishes_a_buyer',
 'ext_per_family',
 ...
 'reproduction_top1',
 'revises_0003_0014_0015_0016_0017_0018_0019_0021',
 'schema',
 'selected_ids',
 'selected_model_id',
 ...
 'verdict']
# 0021's banked model reading on the same rows, for the gain flags only
REF_0021_MODEL_CORRECT = 4587
REF_0021_MODEL_TOP1 = 0.1193
...
def main() -> int:
    ...
    if os.path.exists(NOT_RUN):
        try:
            ...
            print(f"READER 0022: NOT RUN — {nr.get('reason')!r} (stage {nr.get('stage')}, filed {nr.get('utc')}, "
                  f"{nr.get('n_checkpoints')} checkpoints). A NOT RUN is not a verdict.", file=sys.stderr)
        except Exception:  # noqa: BLE001
            print("READER 0022: NOT RUN file present but unreadable.", file=sys.stderr)
        return 2
    if not os.path.exists(ARTIFACT):
        print(f"READER 0022: artifact absent: {os.path.relpath(ARTIFACT, REPO)}\n  Absence is not a pass. No verdict emitted.",
              file=sys.stderr)
        return 2
    try:
        d = json.load(open(ARTIFACT, encoding="utf-8"))
    except Exception as e:  # noqa: BLE001
        print(f"READER 0022: artifact malformed ({type(e).__name__}: {e}).", file=sys.stderr)
        return 2
    try:
        return read(d)
    except Exception as e:  # noqa: BLE001
        # The same key set every other path writes, with None values, so a reading addressed by JSONPath resolves to
        # null rather than to a missing key on this one path (0020 pre-freeze review, reader lens, finding 10).
        result = dict({k: None for k in RESULT_KEYS},
                      schema="raise-v1/realsearch_4096_verdict/1", preregistration=PREREG,
                      ...
                      margin_failed_clauses=[], establishes_a_buyer=False)
        _write(result)
        print(f"READER 0022 — VOID: artifact of unexpected shape ({type(e).__name__}: {e})\n  wrote {os.path.relpath(OUT, REPO)}")
        return 0
...
def read(d) -> int:
    ...
    need = lambda bar: None if bar is None else int(math.ceil(N_REAL * (bar + MARGIN) - 1e-9))  # noqa: E731
    ...
    if roster.get("heads") != HEADS:
        void.append("scope: the banked roster heads are not 0014's sealed heads verbatim")
    ...
    for k in ("pool_chunks_shared_with_ext", "eval_chunks_shared_with_ext", "fit_chunks_shared_with_ext",
              ...
              "fit_rows_identical_to_a_scored_row", "n_chunks_holdout_and_fitpool", "n_holdout_rows_in_fitpool"):
        if part.get(k) != 0:
    ...
    if part.get("null_y_shuffled_sha256") != NULL_SHA_EXPECTED:
        void.append(f"null control: the permuted label array hashes to {part.get('null_y_shuffled_sha256')!r}, not 0021's sealed "
    ...
    # ---- the selection file: selection can be shown never to have held a scored row
    sel = g("selection") or {}
    ...
    gathers = sel.get("gathers")
    if not isinstance(gathers, list) or len(gathers) != 2 or [x.get("name") for x in gathers] != ["holdout", "stage1"] \
            or any(x.get("source") != "fit_corpus" or x.get("n_rows_identical_to_a_scored_row") != 0 for x in gathers) \
            or sel.get("scored_rows_gathered_in_selection") != 0:
        void.append("leakage: stage select did not bank two fit-corpus gathers with 0 scored rows each")
    else:
        if gathers[0].get("n_rows") != PARTITION["n_holdout_rows"] or gathers[0].get("idx_sha256") != PARTITION["holdout_idx_sha256"] \
                or gathers[1].get("n_rows") != PARTITION["stage_rows"][0] or gathers[1].get("idx_sha256") != PARTITION["stage_idx_sha256"][0]:
            void.append("leakage: the gathered holdout or stage block is not the sealed one")
    ...
    def ranked(records):
        pool = [r for r in records if isinstance(r, dict) and r.get("status") == "fit" and num(r.get("top1")) is not None]
        out = []
        while pool:
            best = None
            for r in pool:
    ...
    sel_selected = sel.get("selected_ids") or {}
    model_infeasible = 0
    for hid, h in by_head.items():
        ...
        if len(cands) == 1:
            ...
            if selected.get(hid) != cands[0]["id"] or sel_selected.get(hid) != cands[0]["id"]:
                void.append(f"search: selected_ids[{hid}]={selected.get(hid)!r} / {sel_selected.get(hid)!r} != {cands[0]['id']!r}")
            continue
        stages = entry.get("stages") or []
        if len(stages) != 1:
            void.append(f"search: head {hid} ran {len(stages)} stages, preregistered 1")
            continue
        st = stages[0]
        recs = st.get("records") or []
        expected = [c["id"] for c in cands]
        if [r.get("id") for r in recs if isinstance(r, dict)] != expected or len(recs) != len(expected):
            void.append(f"search: head {hid} records {[r.get('id') for r in recs if isinstance(r, dict)]!r} are not {expected!r} in roster order")
            continue
        if st.get("stage") != 1 or st.get("n_fit_rows") != PARTITION["stage_rows"][0] or st.get("fit_rows_sha256") != PARTITION["stage_idx_sha256"][0]:
            void.append(f"search: head {hid}'s stage is not the sealed stage block")
        for r in recs:
            cid = r.get("id")
            if r.get("status") not in ("fit", "infeasible_memory"):
                void.append(f"search: head {hid} {cid} status {r.get('status')!r}")
            elif r.get("status") != "fit":
                if h.get("side") != "model":
                    void.append(f"search: baseline head {hid} candidate {cid} is {r.get('status')!r}; a symmetric search with a "
                                f"missing baseline arm is not symmetric")
                else:
                    model_infeasible += 1
                    if not r.get("evidence"):
                        void.append(f"search: model candidate {cid} infeasible without evidence")
                    if cid == PROTOCOL["incumbent_id"]:
                        void.append("search: the incumbent is infeasible; the search cannot contain the recipe it was meant to improve on")
            if r.get("seed") != SEED or r.get("n_fit_rows") != PARTITION["stage_rows"][0] or \
                    r.get("fit_rows_sha256") != PARTITION["stage_idx_sha256"][0] or \
                    r.get("fit_rows_sorted_sha256") != PARTITION["stage_sorted_sha256"][0]:
                void.append(f"search: head {hid} {cid} was not fitted on the sealed stage block with the preregistered seed")
            if r.get("params_sha256") != sha_obj(by_id.get(cid)) or r.get("params") != by_id.get(cid, {}).get("params", {}) \
                    or r.get("head") != hid or r.get("stage") != "selection-1":
                void.append(f"search: head {hid} {cid} params hash, params, head or stage is off-roster")
            if r.get("status") == "fit":
                pf = r.get("per_family")
                if num(r.get("top1")) is None or num(r.get("top1_non_gutenberg")) is None or not isinstance(pf, dict) \
                        or sorted(pf) != sorted(FIT_FAMILIES) or any(num(v) is None for v in pf.values()):
                    void.append(f"search: head {hid} {cid} is 'fit' without finite scores over the five real families")
            if any(not rf.get("probe_ok") for rf in (r.get("block_refills") or [])):
                void.append(f"search: head {hid} {cid} banked a failed block probe check")
            renv = r.get("environment") or {}
            for k, v in ENV_FULL.items():
                if renv.get(k) != v:
                    void.append(f"environment: head {hid} {cid} {k}={renv.get(k)!r}, banked {v!r}")
        rk = ranked(recs); keep = PROTOCOL["stages"][0]["keep"]
        if st.get("eligible_ranked_ids") != rk or st.get("advanced_ids") != rk[:keep]:
            void.append(f"search: head {hid} banked ranking/advance differs from the rule's")
        winner = rk[0] if rk else None
        if winner is None:
            void.append(f"search: head {hid} has no eligible candidate")
        if entry.get("selected_id") != winner or selected.get(hid) != winner or sel_selected.get(hid) != winner:
            void.append(f"search: head {hid} selected {entry.get('selected_id')!r} in the selection file and {selected.get(hid)!r} "
                        f"in the artifact; the rule yields {winner!r}")
    ...
    if g("selected_model_id") != selected.get(MODEL_HEAD):
        void.append("search: selected_model_id disagrees with selected_ids for the model head")
    if set(selected) != set(by_head) or set(sel_selected) != set(by_head):
        void.append("search: selected_ids does not name exactly the roster's heads")

    # ---- the reproduction arms: aliased to a head's own fit when that head selected the reference recipe
    aliased_expected = {arm: hid for arm, (hid, ref) in REPRO_ARMS.items() if selected.get(hid) == ref}
    if g("aliased_reproductions") != aliased_expected:
        void.append(f"reproduction: aliased_reproductions={g('aliased_reproductions')!r}, the selected ids give {aliased_expected!r}")
    expected_roles = [n for n in CONFIRM_ORDER if n not in aliased_expected]
    ...
    # ---- confirmatory: one scoring per role, the selected (or reference) recipe, the sealed fit block, the sealed order
    fits = g("fits") or {}
    if not isinstance(fits, dict):
        void.append("fit: fits is not a mapping"); fits = {}

    def role_candidate(name):
        if name in REPRO_ARMS:
            return by_id[REPRO_ARMS[name][1]]
        if name == "null":
            return by_id.get(selected.get(MODEL_HEAD))
        return by_id.get(selected.get(name))

    def role_stage(name):
        return "reproduction" if name in REPRO_ARMS else ("null" if name == "null" else "confirmatory")

    def expected_fp(name, cand):
        return fingerprint(prereg=PREREG, seed=SEED, head=name, cand=cand, stage=role_stage(name), rows=PARTITION["fit_sorted_sha256"],
    ...
    def check_record(name):
        ...
        if rec.get("id") != cand["id"] or rec.get("params_sha256") != sha_obj(cand) or rec.get("params") != cand.get("params", {}) \
            ...
            void.append(f"fit: {name} is not the roster's {cand['id']} (id, params, params hash, family, scaled, val or head off-recipe)")
        if rec.get("seed") != SEED or rec.get("n_fit_rows") != PARTITION["fit_rows"] \
                or rec.get("fit_rows_sha256") != PARTITION["fit_idx_sha256"] or rec.get("fit_rows_sorted_sha256") != PARTITION["fit_sorted_sha256"]:
            void.append(f"fit: {name} was not fitted on the sealed fit block with the preregistered seed")
        if rec.get("stage") != role_stage(name):
            void.append(f"fit: {name} stage {rec.get('stage')!r} is not {role_stage(name)!r}")
        ...
        done = [i for i, e in enumerate(ledger) if isinstance(e, dict) and e.get("name") == f"confirm_{name}" and e.get("event") == "completed"]
        if len(done) != 1:
            ...
            if not any(isinstance(e, dict) and e.get("name") == f"confirm_{name}" and e.get("event") == "started" and e.get("fingerprint") == fp
                       for e in ledger[:done[0]]):
                void.append(f"fit: {name} has no 'started' event with its fingerprint before its completion")
            if not any(isinstance(e, dict) and e.get("name") == f"confirm_{name}" and e.get("event") == "selection_bound"
                       and e.get("selection_sha256") == sel_sha for e in ledger[:done[0]]):
                void.append(f"fit: {name} has no 'selection_bound' event carrying the selection file's hash before its completion")
    ...
    if set(fits) != set(expected_roles):
        extra = sorted(set(fits) - set(expected_roles)); absent = sorted(set(expected_roles) - set(fits))
        void.append(f"fit: the artifact banks roles outside the sealed order (extra {extra}, missing {absent}) - "
                    f"'scored exactly once, in the sealed order' is a property of the run, not of {len(expected_roles)} names")
    ...
    if g("null_rows") != PARTITION["fit_rows"]:
        void.append(f"null control: null_rows={g('null_rows')!r}, the null control is fitted on the whole fit block")
    if len(completed_pos) == len(expected_roles) and [completed_pos[n] for n in expected_roles] != sorted(completed_pos[n] for n in expected_roles):
        void.append("fit: the ledger's completions are not in the sealed order " + ", ".join(expected_roles))
    ...
    if not completions or completions[-1] != f"confirm_{MODEL_HEAD}":
        void.append(f"order: the model's confirmatory fit was not the last completion ({completions[-1:]!r})")
    if g("fit_info_by_name") != {n: (fits.get(n) or {}).get("fit_info") for n in expected_roles}:
    ...
    if ext_fam_idx is not None and fam_e is not None and not any(m.startswith("fit:") or m.startswith("scores:") for m in void):
        ...
        if set(reads) != set(expected_roles):
            void.append(f"readings: banked readings for {sorted(reads)!r}, expected exactly {expected_roles!r}")
        for name in expected_roles:
            if num((g("ext_real_top1") or {}).get(name)) != rec_reads[name]["ext_real_top1"] \
                    ...
                    or (g("ext_correct") or {}).get(name) != rec_reads[name]["ext_correct"] \
        ...
        rm = rec_reads[MODEL_HEAD]
        if num(g("model_real_top1")) != rm["ext_real_top1"] or g("model_real_correct") != rm["ext_real_correct"] \
                or num(g("model_ext_top1")) != rm["ext_top1"] or g("model_ext_correct") != rm["ext_correct"]:
            void.append("readings: the banked model summary readings are not the recomputed values")
        repro_expect = {arm: rec_reads[aliased_expected.get(arm, arm)]["ext_real_top1"] for arm in REPRO_ARMS}
        if {k: num(v) for k, v in (g("reproduction_top1") or {}).items()} != repro_expect:
            void.append("reproduction: reproduction_top1 is not each arm's (or its aliased head's) recomputed real-family reading")
        if g("reference_top1") != REFERENCE:
            void.append("reproduction: the banked reference readings are not 0021's sealed ones")
        drift_expect = {arm: round(repro_expect[arm] - REFERENCE[arm], 6) for arm in REPRO_ARMS}
        if {k: num(v) for k, v in (g("reproduction_drift") or {}).items()} != drift_expect:
            void.append("reproduction: reproduction_drift is not the recomputed drift per arm")
    ...
    # ---- controls: the four reproductions and the null
    if not void:
        for arm in REPRO_ARMS:
            got = rec_reads[aliased_expected.get(arm, arm)]["ext_real_top1"]
            drift = round(got - REFERENCE[arm], 6)
            if abs(drift) > REPRO_TOLERANCE:
                void.append(f"reproduction: {arm} read {got} against 0021's banked {REFERENCE[arm]} (drift {drift} > "
    ...
    # ---- the bars, re-derived: each baseline head floored at 0021's reading, the model with no floor
    bars = g("bars") or {}
    bar_f = bar_e = need_f = need_e = None
    per_head = per_head_searched = None
    if not void:
        per_head_searched = {h: rec_reads[h]["ext_real_top1"] for h in EXPANDED_HEADS}
        per_head = {h: max(per_head_searched[h], FLOORS[h]) for h in EXPANDED_HEADS}

        def best(hs):
            b = max(per_head[h] for h in hs)
            return b, sorted(h for h in hs if per_head[h] == b)[0]
        bar_f, head_f = best(FROZEN_HEADS); bar_e, head_e = best(EXPANDED_HEADS)
        need_f, need_e = need(bar_f), need(bar_e)
        expect_bars = {"per_head": per_head, "per_head_searched": per_head_searched, "floors": FLOORS,
                       "frozen_heads": FROZEN_HEADS, "expanded_heads": EXPANDED_HEADS,
                       "best_frozen_for_bar": bar_f, "best_frozen_head": head_f, "best_frozen_searched": per_head_searched[head_f],
                       "best_expanded_for_bar": bar_e, "best_expanded_head": head_e, "best_expanded_searched": per_head_searched[head_e],
                       "margin": MARGIN, "n_real_rows": N_REAL, "min_correct_real_frozen": need_f, "min_correct_real_expanded": need_e}
        for k in sorted(set(expect_bars) | set(bars)):
            if bars.get(k) != expect_bars.get(k):
                void.append(f"bars: {k}={bars.get(k)!r} is not the reader's recomputation {expect_bars.get(k)!r}")

    fails: list[str] = []
    mreal = rec_reads[MODEL_HEAD]["ext_real_correct"] if not void else None
    if not void:
        if mreal < need_f:
            fails.append(f"margin F (frozen set): the model's correct real-family rows {mreal} of {N_REAL} "
                         f"({rec_reads[MODEL_HEAD]['ext_real_top1']}) is below {need_f} (bar {bar_f} from {head_f} + {MARGIN})")
        if mreal < need_e:
            fails.append(f"margin S (expanded set): {mreal} of {N_REAL} is below {need_e} (bar {bar_e} from {head_e} + {MARGIN})")

    flags = None
    if not void:
        rm = rec_reads[MODEL_HEAD]
        fam_best_frozen = {f: max(rec_reads[h]["ext_per_family"][f] for h in FROZEN_HEADS) for f in REAL_FAMILIES}
        need_fam = {f: int(math.ceil(EXT["rows_per_family"][f] * (fam_best_frozen[f] + MARGIN) - 1e-9)) for f in REAL_FAMILIES}
        fam_clear = [f for f in REAL_FAMILIES if rm["ext_per_family_correct"][f] >= need_fam[f]]
        gain = {"model": round(rm["ext_real_top1"] - REFERENCE["repro_m4"], 6),
                "logistic": round(rec_reads["logistic"]["ext_real_top1"] - REFERENCE["repro_l3"], 6),
                "depth3_tree": round(rec_reads["depth3_tree"]["ext_real_top1"] - REFERENCE["repro_d1"], 6),
                "deep_tree": None}
        n_iter = {n: (fits[n].get("fit_info") or {}).get("n_iter") for n in expected_roles}
        # selection decisiveness and the weighting question, from the banked selection records (informational)
        sel_gap, sel_macro = {}, {}
        for hid, h in by_head.items():
            if len(h["candidates"]) == 1:
                continue
            recs = [r for r in (sel.get("selection") or {}).get(hid, {}).get("stages", [{}])[0].get("records", [])
                    if isinstance(r, dict) and r.get("status") == "fit" and num(r.get("top1")) is not None]
            order_ids = [c["id"] for c in h["candidates"]]
            tops = sorted((r["top1"], -order_ids.index(r["id"])) for r in recs)
            sel_gap[hid] = round(tops[-1][0] - tops[-2][0], 6) if len(tops) >= 2 else None
            macro = [(mean4([num(v) for v in r["per_family"].values()]) if isinstance(r.get("per_family"), dict)
                      and all(num(v) is not None for v in r["per_family"].values()) else None, -order_ids.index(r["id"]), r["id"]) for r in recs]
            macro = [m for m in macro if m[0] is not None]
            sel_macro[hid] = max(macro)[2] if macro else None
        flags = {"frozen_reading_clears": mreal >= need_f, "expanded_reading_clears": mreal >= need_e,
                 "selection_gap_by_head": sel_gap, "selected_ids_under_macro_average": sel_macro,
                 "selection_rule_note": "the sealed rule ranks by the holdout mixture; selected_ids_under_macro_average is what the "
                                        "five per-family holdout top-1s averaged would have chosen, informational",
                 "searched_margin_frozen": round(rm["ext_real_top1"] - bar_f, 6),
                 "searched_margin_expanded": round(rm["ext_real_top1"] - bar_e, 6),
                 "bar_frozen": bar_f, "bar_frozen_head": head_f, "bar_expanded": bar_e, "bar_expanded_head": head_e,
                 "min_correct_real_frozen": need_f, "min_correct_real_expanded": need_e, "n_real_rows": N_REAL,
                 "selected_ids": dict(selected), "aliased_reproductions": aliased_expected,
                 "gain_from_search_by_head": gain,
                 "model_leads_0021_unsearched_model_rows": mreal - REF_0021_MODEL_CORRECT,
                 "ref_0021_model_real_correct": REF_0021_MODEL_CORRECT, "ref_0021_model_real_top1": REF_0021_MODEL_TOP1,
                 "unsearched_0021_margin_over_its_logistic": round(REFERENCE["repro_m4"] - REFERENCE["repro_l3"], 6),
                 "families_clearing": len(fam_clear), "families_clearing_names": fam_clear, "n_real_families": len(REAL_FAMILIES),
                 "per_family_best_frozen_head": fam_best_frozen, "per_family_min_correct": need_fam,
                 "per_family_floor_note": "per-family bars are the best FROZEN head's per-family reading + 0.05 with no sealed "
                                          "per-family floor; informational, never a verdict",
                 "reproduction_drift": {arm: round(rec_reads[aliased_expected.get(arm, arm)]["ext_real_top1"] - REFERENCE[arm], 6)
                                        for arm in REPRO_ARMS},
                 "model_builder_eval_top1": rm["builder_eval_top1"],
                 "model_above_chance_on_builder_eval": rm["builder_eval_top1"] > CHANCE + NULL_TOLERANCE,
                 "synthetic_top1_model": rm["ext_synthetic_top1"],
                 "n_iter_by_name": n_iter,
                 "at_iteration_cap": {n: (n_iter[n] >= MAX_ITER.get(role_candidate(n)["id"], 0)) for n in expected_roles
                                      if _int(n_iter[n]) and MAX_ITER.get(role_candidate(n)["id"], 0) > 0},
                 "fit_info_by_family": {n: role_candidate(n)["family"] for n in expected_roles}}

    if void:
        verdict, meaning = "VOID", ("A validity or control clause fails, or the artifact is incomplete. This run says nothing "
                                    "about a searched recipe on real content in either direction.")
    elif fails:
        verdict, meaning = "REAL_RECIPE_FAILS", (
            f"At 4096 bytes, under a budgeted, symmetric search of 0014's recipe space on {FIT['n_rows']} rows of real content "
            f"from {FIT['n_chunks']} plaintexts of the five pinned real files, the selected model recipe ({selected.get(MODEL_HEAD)}) "
            f"does not beat the best searched trivial baseline on the same rows by {MARGIN}: {mreal} of {N_REAL} real-family rows "
            f"correct ({rec_reads[MODEL_HEAD]['ext_real_top1']}) against {need_f} needed on the frozen set (bar {bar_f} from "
            f"{head_f}) and {need_e} on the expanded set (bar {bar_e} from {head_e}), every baseline floored at 0021's reading "
            f"of it. Searched margins {flags['searched_margin_frozen']} (frozen) and {flags['searched_margin_expanded']} "
            f"(expanded); gains from the search: model {flags['gain_from_search_by_head']['model']}, logistic "
            f"{flags['gain_from_search_by_head']['logistic']}, depth-3 tree {flags['gain_from_search_by_head']['depth3_tree']}. "
            f"{flags['families_clearing']} of {len(REAL_FAMILIES)} real families clear the best frozen head on their own rows by "
            f"{MARGIN} ({', '.join(flags['families_clearing_names']) or 'none'}). An in-distribution reading on real content, not "
            f"a transfer reading: the fit and scored chunks are cut from the same five pinned blobs and share no byte, no chunk "
            f"id and no source-chunk hash. 0021's reading stands and gains a searched sibling. Nothing here revises 0003, 0014, "
            f"0015, 0016, 0017, 0018, 0019 or 0021, and nothing here establishes a buyer.")
    else:
        verdict, meaning = "REAL_RECIPE_CLEARS", (
            f"At 4096 bytes, under a budgeted, symmetric search of 0014's recipe space on {FIT['n_rows']} rows of real content "
            f"from {FIT['n_chunks']} plaintexts of the five pinned real files, the selected model recipe ({selected.get(MODEL_HEAD)}) "
            f"beats the best searched trivial baseline on the same rows by {MARGIN} or more on both readings: {mreal} of {N_REAL} "
            f"real-family rows correct ({rec_reads[MODEL_HEAD]['ext_real_top1']}) against {need_f} needed on the frozen set (bar "
            f"{bar_f} from {head_f}) and {need_e} on the expanded set (bar {bar_e} from {head_e}), every baseline floored at "
            f"0021's reading of it. {flags['families_clearing']} of {len(REAL_FAMILIES)} real families clear the best frozen head "
            f"on their own rows by {MARGIN} ({', '.join(flags['families_clearing_names']) or 'none'}), which is what says whether "
            f"this is a statement about real content or about one family. An IN-DISTRIBUTION reading on real content, not a "
            f"transfer reading: the fit and scored chunks are cut from the same five pinned blobs and share no byte, no chunk "
            f"and no source chunk, but they are the same files. It says the record's recipe was not the right recipe for real "
            f"content, not that the signal would be reachable on a file kind never seen (0018 and 0019 read that, and it failed). "
            f"Nothing here revises 0003, 0014, 0015, 0016, 0017, 0018, 0019 or 0021, and nothing here establishes a buyer.")

    result = {
        "schema": "raise-v1/realsearch_4096_verdict/1",
        ...
        "bars_applied": {"frozen": bar_f, "expanded": bar_e, "per_head": per_head, "per_head_searched": per_head_searched,
                         "floors": FLOORS, "margin": MARGIN, "min_correct_real_frozen": need_f, "min_correct_real_expanded": need_e,
                         "n_real_rows": N_REAL, "fit_rows": FIT["n_rows"], "fit_source_chunks": FIT["n_chunks"]},
        "flags": flags,
        "selected_ids": dict(selected) if not void else None,
        "selected_model_id": selected.get(MODEL_HEAD) if not void else None,
        "model_real_top1": rec_reads[MODEL_HEAD]["ext_real_top1"] if not void else None,
        "model_real_correct": mreal if not void else None,
        "ext_real_top1": {n: rec_reads[n]["ext_real_top1"] for n in expected_roles} if not void else None,
        "ext_real_correct": {n: rec_reads[n]["ext_real_correct"] for n in expected_roles} if not void else None,
        "ext_top1": {n: rec_reads[n]["ext_top1"] for n in expected_roles} if not void else None,
        "ext_per_family": {n: rec_reads[n]["ext_per_family"] for n in expected_roles} if not void else None,
        "ext_subsets": {n: {k: rec_reads[n][k] for k in ("ext_structured_text_top1", "ext_high_entropy_top1",
                                                         "ext_real_top1", "ext_synthetic_top1")} for n in expected_roles} if not void else None,
        "builder_eval_top1": {n: rec_reads[n]["builder_eval_top1"] for n in expected_roles} if not void else None,
        "reproduction_top1": {arm: rec_reads[aliased_expected.get(arm, arm)]["ext_real_top1"] for arm in REPRO_ARMS} if not void else None,
        "reproduction_reference": REFERENCE,
        "reproduction_drift": (flags or {}).get("reproduction_drift"),
        "aliased_reproductions": aliased_expected if not void else None,
        ...
        "informational_only": "the flags, the per-family and subset readings, the reverse readings on the builder's evaluation "
                              "set, the gains from the search and the artifact's cluster intervals are informational: not a "
                              "verdict, not quotable as a pass. The verdict is the `verdict` field, and it is an in-distribution "
                              "reading on real content.",
        "is_a_transfer_reading": False,
        "establishes_a_buyer": False,
        "revises_0003_0014_0015_0016_0017_0018_0019_0021": False,
        "read": {k: g(k) for k in (
            "preregistration", "smoke", "stage", "complete", "missing_roles", "selected_model_id", "selected_ids",
            "aliased_reproductions", "model_real_correct", "model_real_top1", "shuffled_label_accuracy_ext",
    ...
    print("READER 0022 — symmetric recipe search on REAL content at 4096 (in distribution, on the same pinned files)")
    for k, v in result["read"].items():
        print(f"  {k:<40} {v}")
    if not void:
        print(f"  {'ext_real_top1 by role':<40} {json.dumps(result['ext_real_top1'])}")
        print(f"  {'bars (frozen / expanded)':<40} {bar_f} ({flags['bar_frozen_head']}) / {bar_e} ({flags['bar_expanded_head']}); "
              f"need {need_f} / {need_e}")
        print(f"  {'ext_per_family (model)':<40} {json.dumps(result['ext_per_family'][MODEL_HEAD])}")
    if void:
        print(f"\n  VALIDITY FAILED CLAUSES ({len(void)}):")
        for m in void:
            print(f"    · {m}")
    if fails:
        print(f"\n  MARGIN FAILED CLAUSES ({len(fails)}):")
        for m in fails:
            print(f"    · {m}")
    print(f"\n  VERDICT: {verdict}\n  {meaning}")
