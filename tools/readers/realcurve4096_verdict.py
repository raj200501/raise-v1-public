#!/usr/bin/env python3
"""Frozen reader for preregistration 0023 — a scaling curve on REAL plaintexts at 4096.

Written and committed BEFORE any fit was made under this preregistration. Every literal below was generated
from prereg/0023-realcurve-4096.json or read from a banked artifact; none was typed here.

WHY THIS EXISTS. 0021 read REAL_FIT_FAILS and 0022 read REAL_RECIPE_FAILS: on 0021's 26346-row real fit block
the boosted model, the standardised logistic and a depth-3 tree read 0.1193, 0.1179 and 0.0929 on the 38452
sealed real-family rows, and searching the recipes on that content moved nothing. Both named the budget as the
excuse left open. This run cuts 0021's fit block into four nested, family-stratified rungs of plaintexts (1/8,
1/4, 1/2, all), fits 0021's three fixed recipes on each, scores each fit once on 0018's scoring set, and reads
each role's real-family top-1 against log2(plaintexts): an ordinary-least-squares slope in accuracy per doubling
of plaintexts, with a cluster bootstrap over scored chunks (the same resamples for every role, so role
differences are paired). The top rung IS 0021's fit block row for row; its three roles are the reproduction. A
replicate of the smallest rung under a second seeded nesting is fitted last, informational: the direct reading of
the fit-side variance the scored-chunk bootstrap cannot see.

This reader takes the runner's artifact (tools/pivot/run_realcurve.py) and checks, clause by clause: the sealed
corpora (array hashes for the builder cache, the evaluation corpus and the fit corpus); the sealed split, the
sealed fit block and every sealed rung (hash for hash, nested, strictly increasing, topped by the whole block);
that nothing scored was fitted (the runner's measured chunk, source-chunk and row-identity counts must all be
0); that every fit is the sealed recipe on the sealed rung with the sealed seed, scored exactly once, in the
sealed order, with its fingerprint recomputed here; that each record's own scores equal what its per-example
vector gives; that the top rung's three roles land within 0.005 of 0021's banked values; that the null control
sits at chance on both row sets; that every banked reading, every banked curve, slope, interval and replicate
equals what this reader recomputes from the vectors and the sealed chunk layout. Then two clauses on the
reader's own numbers: the model's slope >= 0.005 per doubling of plaintexts, and the 2.5th percentile of that
slope under the sealed cluster bootstrap > 0. REAL_CURVE_RISES or REAL_CURVE_FLAT, published at the same size;
VOID on any validity failure.

NOT A TRANSFER READING. The fit block and the scored chunks are cut from the same five pinned blobs; they share
no byte, no chunk index, no chunk id and no source-chunk hash, and the runner measures that rather than assuming
it. Every verdict string this reader emits says so, and a pass is stated as a rise to 0021's failing reading.
...
cross-checks whenever the checkpoint directory is present; it also requires the sixteen vectors to be pairwise
distinct. No extrapolation beyond the top rung is banked.
...
ARTIFACT = os.path.join(REPO, "artifacts", "pivot", "realcurve_4096.json")
SCORES = os.path.join(REPO, "artifacts", "pivot", "realcurve_4096_scores.json")
NOT_RUN = os.path.join(REPO, "artifacts", "pivot", "realcurve_4096_not_run.json")
OUT = os.path.join(REPO, "artifacts", "pivot", "realcurve_4096_verdict.json")
PREREG = "0023-realcurve-4096"
...
PROTOCOL = {'seed': 20260825,
 ...
 'caps_note': "0022's caps verbatim; selection_fit_seconds is dormant here (every fit is confirmatory) and "
              'memory_kill_gb is the live one',
 ...
 'reference_source': 'artifacts/pivot/realfit_4096_rerun.json ext_real_top1: real_model (M4), real_logistic '
                     "(L3), floor_tree3 (D1), each fitted on this fit block in 0021's order and scored on this "
                     'scoring set; 0022 reproduced all three with drift 0.0. The top rung IS that fit block '
                     'row for row, so its three roles are the reproduction arms.',
 ...
 'null_note': "the null control permutes the whole fit block's labels with the split's own generator, exactly "
              "as 0021 and 0022 did at the same generator state, so the permuted label array is 0021's and its "
              'hash is sealed here; it is the first role in the order so that no other draw precedes it',
 ...
 'replicate_rung_1_hashes': {'rung': 'rep1',
                             'replicates_rung': 1,
                             'denominator': 8,
                             'n_chunks': 130,
                             'n_rows': 3345,
                             'chunks_per_family': {'c_src': 3,
                                                   'pe_bin': 26,
                                                   'py_src': 84,
                                                   'rfc_txt': 12,
                                                   'rst_doc': 5},
                             'rows_per_family': {'c_src': 78,
                                                 ...
                                                 'rst_doc': 130},
                             'log2_chunks': 7.022368,
                             'idx_sha256': '4ef765abd6cee596f4144939fe9acb33ce71ca15039d42bbdff8f149f3bba466',
                             'sorted_sha256': '80e3a896c44fa53446b53ae0c2c500c840dfc28bc66314004a38f5ca6aa7b23f',
                             'chunks_sha256': '26fb61d387e6692785eda5814dbdb51cf0a5f01728e2c12f3342102fa184e837',
                             'chunks_shared_with_rung_1': 16,
                             'rule': "the smallest rung's rule and denominator under "
                                     'np.random.default_rng([seed, family_index + 1, 1]); informational: a '
                                     'direct reading of how much the smallest rung moves under a second draw '
                                     'of plaintexts'},
 ...
 'bar': {'slope_per_doubling': 0.005, 'bootstrap_lower_bound_gt': 0.0},
 'bootstrap': {'n_boot': 2000,
               'seed': 20260825,
               'rule': 'cluster bootstrap over the SCORED real-family chunks: np.random.default_rng(seed) '
                       "draws n chunks with replacement per resample; every rung's accuracy on the resample is "
                       'rounded to 4 decimals and the OLS slope against log2(plaintexts) refitted; one '
                       'resample serves every rung and every role, so role differences are paired; the 2.5th '
                       'and 97.5th percentiles are the interval. The interval is over the scored chunks only: '
                       'it says how far the slope moves if a different sample of held-out chunks of the same '
                       'files is scored. It does not resample the fit side - which plaintexts fell into the '
                       'lower rungs (one seed, one nesting) or where early stopping halted - and is not an '
                       "interval on the slope of a re-drawn curve. The replicate rung is the run's only direct "
                       'reading of that side, at the smallest rung, and it is informational.'},
 'reference_slopes_informational': {'builder_per_plaintext_slope_per_doubling': 0.010093,
                                    'builder_per_plaintext_is': "0021's builder_chunk_matched arm on the "
                                                                "builder's evaluation set (0.1862 at 1029 "
                                                                'builder plaintexts, '
                                                                'artifacts/pivot/realfit_4096_rerun.json) to '
                                                                "0003's top rung (0.2395 at 40000 pool "
                                                                'plaintexts, '
                                                                'artifacts/pivot/deflate_curve.json): 5.2807 '
                                                                'doublings of plaintexts with rows growing '
                                                                'with them - the in-distribution per-plaintext '
                                                                "rate on the builder's content",
                                    'builder_fixed_rows_plaintext_slope_per_doubling': 0.003492,
                                    'builder_fixed_rows_is': "0021's builder_chunk_matched (0.1862, 1029 "
                                                             'plaintexts) to builder_matched (0.201, 19413 '
                                                             'plaintexts) at the same 26346 rows: the '
                                                             'plaintext-only effect at a fixed row budget on '
                                                             "the builder's content",
                                    'builder_mixed_family_slope_per_doubling': 0.000854,
                                    'builder_mixed_family_is': "0003's per-family curve for the builder's "
                                                               "heterogeneous 'mixed' family "
                                                               '(artifacts/pivot/per_family_curves.json: '
                                                               "{'1000': 0.1059, '10000': 0.1135, '100000': "
                                                               "0.1154, '800000': 0.1143} over the four rungs, "
                                                               "0.002837 per decade): the record's closest "
                                                               'in-distribution analogue of heterogeneous '
                                                               'content, and it is flat',
                                    'builder_mixture_slope_per_doubling_of_fragments': 0.014787,
                                    'builder_mixture_is': "0003's headline slope 0.049121 per decade of "
                                                          'manufactured FRAGMENTS over 2.9031 decades, '
                                                          "converted; a different abscissa (0003's low rungs "
                                                          "hold about one row per plaintext, this run's about "
                                                          '25.6)',
                                    'note': 'informational references for the prose and for one flag (the '
                                            "model's slope as a fraction of "
                                            'builder_per_plaintext_slope_per_doubling); none is a clause'}}
PROTOCOL_SHA256 = "3d57793498957b228abfb2f6f0f4bcb8eaddb77a42c2425e77839b3817bdf274"
CURVE_SHA256 = "7300a288139d2726fef353b8ac0d021dd881a4bf28d999b061cd2eb15959a1e7"
RECIPES = {'model': {'id': 'M4',
           'family': 'hgb',
           'val': 'frag',
...
ROLES = ["depth3_tree", "logistic", "model"]
ORDER = ["null", "r4_depth3_tree", "r4_logistic", "r4_model", "r1_depth3_tree", "r1_logistic", "r1_model", "r2_depth3_tree", "r2_logistic", "r2_model", "r3_depth3_tree", "r3_logistic", "r3_model", "rep1_depth3_tree", "rep1_logistic", "rep1_model"]
DENOMINATORS = [8, 4, 2, 1]
N_RUNGS = 4
TOP = "r4_"
...
PARTITION = {'seed': 20260825,
 ...
 'top_rung_is_fit_block': True,
 'doublings_spanned': 2.984659,
 'replicate_rung_1': {'rung': 'rep1',
                      'replicates_rung': 1,
                      'denominator': 8,
                      'n_chunks': 130,
                      'n_rows': 3345,
                      ...
                      'chunks_sha256': '26fb61d387e6692785eda5814dbdb51cf0a5f01728e2c12f3342102fa184e837',
                      'chunks_shared_with_rung_1': 16,
                      'rule': "the smallest rung's rule and denominator under np.random.default_rng([seed, "
                              'family_index + 1, 1]); informational: a direct reading of how much the smallest '
                              'rung moves under a second draw of plaintexts'},
...
N_EXT = 61409
N_REAL = 38452
REFERENCE = {'model': 0.1193, 'logistic': 0.1179, 'depth3_tree': 0.0929}              # 0021's banked readings the top rung's three roles must reproduce
...
BAR_SLOPE = 0.005              # accuracy on the real-family rows per doubling of plaintexts
BOOT_LB = 0.0                  # the 2.5th percentile of the model's slope must exceed this
N_BOOT = 2000
BOOT_SEED = 20260825
LOG2X = [7.022368, 8.011227, 9.008429, 10.007027]                      # log2(plaintexts) per rung, sealed
MAX_ITER = {'depth3_tree': 0, 'logistic': 1000, 'model': 250}                # by role: the iteration cap of the hgb and logistic recipes
REPLICATE = "rep1"            # the smallest rung cut again under a second nesting, fitted last; informational
REF_BUILDER_SLOPE = PROTOCOL["reference_slopes_informational"]["builder_per_plaintext_slope_per_doubling"]
CKPT_DIR = os.path.join(REPO, "artifacts", "pivot", "realcurve_4096_ckpt")
RESULT_KEYS = ['bar_applied',
 ...
 'reproduction_top1',
 'revises_0003_0014_0015_0016_0017_0018_0019_0021_0022',
 'schema',
...
def main() -> int:
    ...
    if os.path.exists(NOT_RUN):
        try:
            ...
            print(f"READER 0023: NOT RUN — {nr.get('reason')!r} (stage {nr.get('stage')}, filed {nr.get('utc')}, "
                  f"{nr.get('n_checkpoints')} checkpoints). A NOT RUN is not a verdict.", file=sys.stderr)
        except Exception:  # noqa: BLE001
            print("READER 0023: NOT RUN file present but unreadable.", file=sys.stderr)
        return 2
    if not os.path.exists(ARTIFACT):
        print(f"READER 0023: artifact absent: {os.path.relpath(ARTIFACT, REPO)}\n  Absence is not a pass. No verdict emitted.",
              file=sys.stderr)
        return 2
    try:
        d = json.load(open(ARTIFACT, encoding="utf-8"))
    except Exception as e:  # noqa: BLE001
        print(f"READER 0023: artifact malformed ({type(e).__name__}: {e}).", file=sys.stderr)
        return 2
    try:
        return read(d)
    except Exception as e:  # noqa: BLE001
        result = dict({k: None for k in RESULT_KEYS},
                      schema="raise-v1/realcurve_4096_verdict/1", preregistration=PREREG,
        ...
        _write(result)
        print(f"READER 0023 — VOID: artifact of unexpected shape ({type(e).__name__}: {e})\n  wrote {os.path.relpath(OUT, REPO)}")
        return 0
...
def read(d) -> int:
    ...
    def rung_of(name):
        ...
        return REPLICATE if head == REPLICATE else int(head[1:])
    ...
    def rk_of(k):
        return PARTITION["replicate_rung_1"] if k == REPLICATE else rungs_sealed[k - 1]

    def stage_of(name):
        return "null" if name == "null" else ("replicate" if rung_of(name) == REPLICATE else "curve")
    ...
    if g("roles") != ROLES or g("n_rungs") != N_RUNGS or g("denominators") != DENOMINATORS or g("log2_chunks") != LOG2X:
        void.append(f"scope: roles={g('roles')!r}, n_rungs={g('n_rungs')!r}, denominators={g('denominators')!r}, "
                    f"log2_chunks={g('log2_chunks')!r} are not the sealed ones")
    ...
    for k in ("rungs_nested", "rung_chunks_strictly_increasing", "top_rung_is_fit_block", "split_is_grouped_by_source"):
    ...
    def expected_fp(name, cand):
        ...
        return fingerprint(prereg=PREREG, seed=SEED, head=("null" if name == "null" else role_of(name)), cand=cand,
    ...
    if os.path.isdir(CKPT_DIR):
        ...
        for name in ORDER:
            ...
            ok_ = (isinstance(ck, dict) and ck.get("fingerprint") == expected_fp(name, cand_of(name)) and _ints(pe, n_all, (0, 1))
                   ...
                   and (ck.get("record") or {}).get("top1") == rec.get("top1"))
            if not ok_:
                void.append(f"checkpoint: {name}'s committed checkpoint does not carry the sealed fingerprint, the banked vector and the record's top-1")
            else:
                ckpt_checked += 1
    ...
    if ext_fam_idx is not None and fam_e is not None and not any(m.startswith("fit:") or m.startswith("scores:") for m in void):
        ...
        repro_expect = {role: rec_reads[TOP + role]["ext_real_top1"] for role in ROLES}
        if {k: num(v) for k, v in (g("reproduction_top1") or {}).items()} != repro_expect:
            void.append("reproduction: reproduction_top1 is not the top rung's recomputed real-family reading per role")
    ...
    # ---- controls: the top rung reproduces 0021; the null sits at chance
    if not void:
        for role in ROLES:
            got = rec_reads[TOP + role]["ext_real_top1"]; drift = round(got - REFERENCE[role], 6)
            if abs(drift) > REPRO_TOLERANCE:
                void.append(f"reproduction: {TOP}{role} read {got} against 0021's banked {REFERENCE[role]} (drift {drift} > "
    ...
    # ---- the curve, re-derived: rung readings, slopes, gains, per-family slopes, the sensitivity, the replicate, the bootstrap
    curves_banked = g("curves") or {}
    my_curves = {}; boot = None; reader_slope = None; reader_lb = None; my_rep = None
    if not void:
        for role in ROLES:
            pts = []
            for k, rk in enumerate(rungs_sealed, start=1):
                rr = rec_reads[f"r{k}_{role}"]
                pts.append({"rung": k, "denominator": rk["denominator"], "n_chunks": rk["n_chunks"], "n_rows": rk["n_rows"],
            ...
            c = {"rungs": pts, "complete": True, "slope_per_doubling": round(ols_slope(LOG2X, ys), 6),
                 "end_to_end_gain": round(ys[-1] - ys[0], 4),
                 "rung_to_rung_gains": [round(ys[i + 1] - ys[i], 4) for i in range(len(ys) - 1)],
                 "slope_with_top_rung_at_reference": round(ols_slope(LOG2X, ys[:-1] + [float(REFERENCE[role])]), 6),
                 "per_family_slope_per_doubling": {f: round(ols_slope(LOG2X, [p["ext_per_family"][f] for p in pts]), 6) for f in REAL_FAMILIES},
                 "builder_eval_slope_per_doubling": round(ols_slope(LOG2X, [p["builder_eval_top1"] for p in pts]), 6)}
            my_curves[role] = c
        ...
        ms = my_curves["model"]["slope_per_doubling"]; ls = my_curves["logistic"]["slope_per_doubling"]; ts = my_curves["depth3_tree"]["slope_per_doubling"]
        if num(g("model_slope_per_doubling")) != ms or num(g("logistic_slope_per_doubling")) != ls or num(g("depth3_tree_slope_per_doubling")) != ts:
            void.append("curve: a banked top-level slope is not the recomputed role slope")
        ...
        reader_slope = ms
        # the replicate of the smallest rung, re-derived
        my_rep = {"rung": PARTITION["replicate_rung_1"], "readings": {}, "difference_from_rung_1": {}, "slope_with_replicate_at_rung_1": {}}
        for role in ROLES:
            rr_ = rec_reads[f"{REPLICATE}_{role}"]; r1 = rec_reads[f"r1_{role}"]
            my_rep["readings"][role] = {"ext_real_top1": rr_["ext_real_top1"], "ext_real_correct": rr_["ext_real_correct"],
                                        "builder_eval_top1": rr_["builder_eval_top1"],
                                        "ext_per_family": {f: rr_["ext_per_family"][f] for f in REAL_FAMILIES}}
            my_rep["difference_from_rung_1"][role] = round(rr_["ext_real_top1"] - r1["ext_real_top1"], 4)
            ys = [p["ext_real_top1"] for p in my_curves[role]["rungs"]]
            my_rep["slope_with_replicate_at_rung_1"][role] = round(ols_slope(LOG2X, [rr_["ext_real_top1"]] + ys[1:]), 6)
        rb = g("replicate_rung_1") or {}
        for kk in list(my_rep):
            if rb.get(kk) != my_rep[kk]:
                void.append(f"replicate: replicate_rung_1.{kk} is not the reader's recomputation")
        if not isinstance(rb.get("note"), str):
            void.append("replicate: replicate_rung_1 carries no note")
        ...
        by_role = {role: [np.asarray([scores[f"r{k}_{role}"][n_eval + i] for i in real_idx], np.int8) for k in range(1, N_RUNGS + 1)]
                   for role in ROLES}
        sl = slope_bootstrap(by_role, LOG2X, cid, N_BOOT, BOOT_SEED)
    ...
    if not void:
        if reader_slope < BAR_SLOPE - 1e-9:
            fails.append(f"slope: the model's real-family accuracy rises {reader_slope} per doubling of plaintexts over "
                         f"{PARTITION['doublings_spanned']} doublings ({[p['ext_real_top1'] for p in my_curves['model']['rungs']]}), below the "
                         f"preregistered {BAR_SLOPE}")
        if reader_lb <= BOOT_LB:
            fails.append(f"slope_bootstrap: the 2.5th percentile of the model's slope under the cluster bootstrap is {reader_lb}, not above "
                         f"{BOOT_LB} (interval {boot['model']['ci95']}); four points and a rise the scored chunks do not agree on is not a rise")
    ...
    if not void:
        ...
        rung_table = [{"rung": k, "denominator": rk["denominator"], "n_chunks": rk["n_chunks"], "n_rows": rk["n_rows"],
                       **{role: rec_reads[f"r{k}_{role}"]["ext_real_top1"] for role in ROLES}}
                      for k, rk in enumerate(rungs_sealed, start=1)]
        lo, hi = boot["model_minus_logistic"]["ci95"]
        if lo > 0:
            diff_read = ("the paired interval is entirely above 0: on this content the model buys more per doubling than the linear rule - "
                         "and, because the top rung pins model minus logistic at 0021's reading, only because the linear rule led at the "
                         "small rungs and the model caught up")
        elif hi < 0:
            diff_read = "the paired interval is entirely below 0: on this content the linear rule buys more per doubling than the model"
        else:
        ...
        flags = {"slope_clears": reader_slope >= BAR_SLOPE - 1e-9, "bootstrap_clears": reader_lb > BOOT_LB,
                 "model_slope_per_doubling": reader_slope, "model_slope_ci95": boot["model"]["ci95"],
                 "logistic_slope_per_doubling": my_curves["logistic"]["slope_per_doubling"], "logistic_slope_ci95": boot["logistic"]["ci95"],
                 "depth3_tree_slope_per_doubling": my_curves["depth3_tree"]["slope_per_doubling"], "depth3_tree_slope_ci95": boot["depth3_tree"]["ci95"],
                 "slope_model_minus_logistic": round(reader_slope - my_curves["logistic"]["slope_per_doubling"], 6),
                 ...
                 "slope_model_minus_depth3_tree": round(reader_slope - my_curves["depth3_tree"]["slope_per_doubling"], 6),
                 "slope_model_minus_depth3_tree_ci95_paired": boot["model_minus_depth3_tree"]["ci95"],
                 "end_to_end_gain": {role: my_curves[role]["end_to_end_gain"] for role in ROLES},
                 "rung_to_rung_gains": {role: my_curves[role]["rung_to_rung_gains"] for role in ROLES},
                 "per_family_slope_per_doubling": {role: my_curves[role]["per_family_slope_per_doubling"] for role in ROLES},
                 "builder_eval_slope_per_doubling": {role: my_curves[role]["builder_eval_slope_per_doubling"] for role in ROLES},
                 "slope_with_top_rung_at_0021s_reading": {role: my_curves[role]["slope_with_top_rung_at_reference"] for role in ROLES},
                 "builder_per_plaintext_slope_per_doubling_reference": REF_BUILDER_SLOPE,
                 "model_slope_as_fraction_of_builder_per_plaintext_slope": (round(reader_slope / REF_BUILDER_SLOPE, 4) if REF_BUILDER_SLOPE else None),
                 "replicate_rung_1": {"readings_ext_real_top1": {role: my_rep["readings"][role]["ext_real_top1"] for role in ROLES},
                                      "difference_from_rung_1": my_rep["difference_from_rung_1"],
                                      "slope_with_replicate_at_rung_1": my_rep["slope_with_replicate_at_rung_1"],
                                      "chunks_shared_with_rung_1": PARTITION["replicate_rung_1"]["chunks_shared_with_rung_1"],
                                      "note": "informational: one replicate reads the size of the fit-side variance at the smallest rung; "
                 ...
                 "logistic_leads_model_at_every_rung": all(rec_reads[f"r{k}_logistic"]["ext_real_top1"] > rec_reads[f"r{k}_model"]["ext_real_top1"]
                                                          for k in range(1, N_RUNGS + 1)),
                 "top_rung_is_0021s_failing_reading": {"model": rec_reads[TOP + "model"]["ext_real_top1"],
                 ...
                 "reproduction_drift": {role: round(rec_reads[TOP + role]["ext_real_top1"] - REFERENCE[role], 6) for role in ROLES},
    ...
    if void:
        verdict, meaning = "VOID", ("A validity or control clause fails, or the artifact is incomplete. This run says nothing "
                                    "about a scaling curve on real plaintexts in either direction.")
    else:
        common = (f"At 4096 bytes, on nested rungs of {plaintexts[0]} to {plaintexts[-1]} real plaintexts of the five pinned real files "
                  f"({PARTITION['doublings_spanned']} doublings), 0021's three fixed recipes read, per rung: the model {by_rung['model']}, the "
                  f"standardised logistic {by_rung['logistic']}, the depth-3 tree {by_rung['depth3_tree']} on the {N_REAL} real-family rows. "
                  f"Slopes per doubling of plaintexts: model {reader_slope} (bootstrap interval over the scored chunks {boot['model']['ci95']}), "
                  f"logistic {flags['logistic_slope_per_doubling']} ({boot['logistic']['ci95']}), depth-3 tree {flags['depth3_tree_slope_per_doubling']} "
                  f"({boot['depth3_tree']['ci95']}); model minus logistic {flags['slope_model_minus_logistic']}, paired interval "
                  f"{flags['slope_model_minus_logistic_ci95_paired']}: {flags['difference_reading']}. The replicate of the smallest rung under a "
                  f"second nesting reads {flags['replicate_rung_1']['readings_ext_real_top1']} (differences from rung 1 "
                  f"{flags['replicate_rung_1']['difference_from_rung_1']}). The curve's top rung is 0021's reading, which fails the record's "
                  f"margin ({top_read} against {flags['top_rung_is_0021s_failing_reading']['needed_over_the_tree_at_0021']} needed over the "
                  f"depth-3 tree, {flags['top_rung_is_0021s_failing_reading']['margin_over_the_logistic_at_0021']} over the logistic at 0021). "
                  f"An in-distribution reading on real content, not a transfer reading: the fit and scored chunks are cut from the same five "
                  f"pinned blobs and share no byte, no chunk id and no source-chunk hash; the interval is over the scored chunks only and "
                  f"does not resample the fit side. Nothing here revises 0003, 0014, 0015, 0016, 0017, 0018, 0019, 0021 or 0022, and nothing "
        ...
        if fails:
            ...
            verdict, meaning = "REAL_CURVE_FLAT", (
                f"REAL_CURVE_FLAT on {which}: the model's real-family accuracy does not rise with the plaintexts fitted at the preregistered "
                f"rate of {BAR_SLOPE} per doubling with a scored-chunk interval above {BOOT_LB} - "
                ...
                + ". " + common + " The budget excuse closes on this content at the record's rate; it says nothing about a rate below it.")
        else:
            verdict, meaning = "REAL_CURVE_RISES", (
                f"REAL_CURVE_RISES: the model's real-family accuracy rises with the plaintexts fitted at or above {BAR_SLOPE} per doubling, "
                f"with the scored-chunk interval above {BOOT_LB} - a rise to 0021's failing reading, never 'more real data fixes the model'. "
                + common + " A rise the logistic and the tree share is a rise of the cheap signal, not of the recipe. A curve of just under "
                f"one decade on the only real content the record has outside its sealed scoring set; nothing beyond the top rung is banked.")

    result = {
        "schema": "raise-v1/realcurve_4096_verdict/1",
        ...
        "bar_applied": {"slope_per_doubling": BAR_SLOPE, "bootstrap_lower_bound_gt": BOOT_LB, "n_boot": N_BOOT, "bootstrap_seed": BOOT_SEED,
                        "doublings_spanned": PARTITION["doublings_spanned"], "log2_chunks": LOG2X,
                        "rungs": [{k: r[k] for k in ("rung", "denominator", "n_chunks", "n_rows")} for r in rungs_sealed],
        ...
        "logistic_slope_per_doubling": my_curves["logistic"]["slope_per_doubling"] if not void else None,
        "depth3_tree_slope_per_doubling": my_curves["depth3_tree"]["slope_per_doubling"] if not void else None,
        ...
        "replicate_rung_1": my_rep if not void else None,
        "flags": flags,
        ...
        "reproduction_top1": {role: rec_reads[TOP + role]["ext_real_top1"] for role in ROLES} if not void else None,
        ...
        "informational_only": "the flags, the other roles' slopes and intervals, the per-family slopes, the replicate, the sensitivity "
                              "slopes, the reverse curves on the builder's evaluation set and the artifact's cluster intervals are "
                              "informational: not a verdict, not quotable as a pass. The verdict is the `verdict` field, read on the "
                              "model's slope and its scored-chunk interval, and it is an in-distribution reading on real content whose "
                              "top rung fails the record's margin.",
        "is_a_transfer_reading": False,
        "establishes_a_buyer": False,
        "revises_0003_0014_0015_0016_0017_0018_0019_0021_0022": False,
        "read": {k: g(k) for k in (
            ...
            "n_rungs", "curve_sha256")},
        "readings_banked": reads,
    }
    _write(result)

    print("READER 0023 — a scaling curve on REAL plaintexts at 4096 (in distribution, on the same pinned files)")
    ...
    if not void:
        ...
        print(f"  {'slopes (model / logistic / tree)':<40} {reader_slope} {boot['model']['ci95']} / {flags['logistic_slope_per_doubling']} / "
              f"{flags['depth3_tree_slope_per_doubling']}; model minus logistic {flags['slope_model_minus_logistic']} "
              f"{flags['slope_model_minus_logistic_ci95_paired']}")
        print(f"  {'replicate of rung 1 (diff from rung 1)':<40} {flags['replicate_rung_1']['difference_from_rung_1']}")
    if void:
