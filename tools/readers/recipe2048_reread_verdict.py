#!/usr/bin/env python3
"""Frozen reader for preregistration 0013 — a RE-READ of 0012's banked artifact under a reader whose
one defect is corrected.

WHAT HAPPENED. 0012's frozen reader (tools/readers/recipe2048_verdict.py, sealed in chain entry 12)
emitted VOID on an honest, complete run for a reason that lies in the reader, not in the run: for the
second selection stage it expected the surviving candidates' records in RANKED order, while the runner
fits survivors in ROSTER order (the order every other clause of that reader uses). Both orderings hold
the same records, the same ranking and the same winner. The 0012 mutation gate did not catch this
because its synthetic artifact was built to the reader's assumption rather than to the runner's
behaviour; that is filed in CORRECTIONS.md. A frozen reader is not edited after it has seen a number,
so 0012 stands VOID on the record, and this reader re-reads the SAME banked artifact under the SAME
clauses with exactly one line changed: the stage-2 records are expected in roster order.

DISCLOSED IN FULL. Every 0012 number was known when this reader and its preregistration were written:
the selected model M7 scores 0.212 (0.2223 without gutenberg), the searched logistic 0.1886 (0.1975),
the incumbent refit 0.1741, the 0011 logistic refit 0.1266, the null control 0.0408. Under the clauses
below those numbers read RECIPE_FAILS on all three margins (0.0234, 0.0234, 0.0248 against 0.05). This
preregistration therefore commits, before the reader is hashed into the chain, to the outcome the known
numbers give; its only freedom would have been to change a clause, and no clause is changed. It cannot
revise 0012's VOID, 0011, 0007, 0006 or 0003, and it establishes no buyer.

Everything after this paragraph is 0012's reader verbatim except the one line marked "survivors in
ROSTER order", the output path, and the preregistration stamp the verdict carries (the artifact it
reads is still required to be stamped 0012-recipe-search-2048).

--- 0012 READER DOCSTRING, RETAINED ---
...
OUT = os.path.join(REPO, "artifacts", "pivot", "recipe_search_2048_reread_verdict.json")

PREREG = "0012-recipe-search-2048"          # the artifact this reader reads is 0012's
REREAD = "0013-recipe-search-2048-reread"   # this reader's own preregistration
...
def main() -> int:
    ...
    result = {
        "schema": "raise-v1/recipe_search_2048_reread_verdict/1",
    ...
    print("READER 0013 — re-read of 0012's banked artifact (symmetric recipe search at 2048)")
