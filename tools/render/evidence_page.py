...
# 0018 (out-of-builder transfer) is rendered only once its artifact and frozen verdict exist.
...
# The stamp names the commit the page was rendered at; when artifacts are uncommitted at render
# time (the normal case: render, then commit) it says so, and the artifact hash below is the
# provenance that does not depend on commit timing (board review 2026-09-03).
_dirty = subprocess.run(["git", "status", "--porcelain", "--", "artifacts", "tools/render"], cwd=REPO,
                        capture_output=True, text=True).stdout.strip()
if _dirty:
    git_head += "+uncommitted-artifacts"
import hashlib as _hl
_h = _hl.sha256()
for _p in sorted(__import__("glob").glob(os.path.join(REPO, "artifacts", "**", "*.json"), recursive=True)):
    _h.update(open(_p, "rb").read())
artifacts_sha = _h.hexdigest()[:16]
...
chips = "".join([
 ...
 + ([chip(rfv["verdict"], {"REAL_FIT_CLEARS": "pass", "REAL_FIT_FAILS": "fail", "VOID": "inc"}[rfv["verdict"]], "0021",
           (f'the same recipes fitted on {rfv["bar_applied"]["fit_rows"]} rows from '
            f'{rfv["bar_applied"]["fit_source_chunks"]} plaintexts of REAL content - the chunks of 0018\'s five pinned '
            f'real files its sealed evaluation corpus does not use - and read IN DISTRIBUTION on those sealed rows. Real '
            f'content DOES carry the signal: {rfv["ext_real_correct_model"]} of {rfv["bar_applied"]["n_real_rows"]} '
            f'correct ({rfv["ext_real_top1_model"]}), nearly double 0018\'s builder-fitted '
            f'{rfv["bar_applied"]["flags"]["oob_0018_run_read_by_0019_builder_fit_real_top1"]} on the same rows. But a '
            f'depth-3 tree on the same rows reads {rfv["bar_applied"]["floor_real_top1"]}, and the clause is floor + '
            f'{rfv["bar_applied"]["bar"]} (OPERATING_RULES 4a), not chance: {rfv["bar_applied"]["min_correct_real_over_floor"]} '
            f'needed. Under the chance bar ({rfv["bar_applied"]["min_correct_real_chance_based"]}) it would have passed. '
            f'{rfv["bar_applied"]["families_clearing"]} of 5 families clear their own floor; both builder-matched arms read '
...
if os.path.exists(_pf2048):
    _pfdoc = json.load(open(_pf2048, encoding="utf-8")); pf = _pfdoc["families"]; pfsub = _pfdoc["subsets"]
    ...
    fam2048_table = (f'<p><strong>Where the 2048 near-miss lives</strong> — a decomposition from the banked scores, '
                     ...
                     f'readings; three incompressible families and the gutenberg prose family fail on every reading; the mixture '
                     f'without base64, binary and mixed still misses the binding reading ({pfsub["without_base64_binary_mixed"]["margin_expanded"]["value"]:+.4f}); the verdict is the mixture\'s.</p>'
...
if fdc and fdcv and fdcv["verdict"] != "VOID" and fdc.get("complete"):
    ...
    boundary_0016 = (f'<p><strong>Spreading a fixed budget over more content families {_word} transfer: '
                     ...
                     f'{fdcv["fdc_mixture_top1_by_k_logistic"]} (slope {fdcv["fdc_slope_per_doubling_logistic"]}) and 0014\'s standardised '
                     f'logistic {fdcv["fdc_mixture_top1_by_k_logistic_l3"]} (slope {fdcv["fdc_slope_per_doubling_logistic_l3"]}). A single family '
...
if rfa and rfv and rfv["verdict"] != "VOID" and rfa.get("complete"):
    ...
    boundary_0020 = (f'<p><strong>Fitted on real files, the signal is there — and a depth-3 tree gets most of it: '
                     f'<span class="mono">{rfv["verdict"]}</span></strong> (preregistration 0021, chain entry 21, read by '
                     f'its own frozen reader, which recounts every reading from the banked score vectors). The three '
                     f'recipes were fitted on {_ba["fit_rows"]} rows from {_ba["fit_source_chunks"]} plaintexts of real '
                     f'content — the chunks of 0018\'s five pinned real files that its sealed evaluation corpus does not '
                     f'use, disjoint by index, by chunk id, by source-chunk hash and by feature-row bytes — and scored '
                     f'once on 0018\'s scoring set. <strong>Real content does carry the encoder signal.</strong> The '
                     f'headline recipe identifies the encoder on {rfv["ext_real_correct_model"]} of {_ba["n_real_rows"]} '
                     f'real-family rows ({rfv["ext_real_top1_model"]}) — '
                     f'{round(rfv["ext_real_top1_model"]/_ba["chance"],2)}&times; chance, and nearly double the '
                     f'{_fl["oob_0018_run_read_by_0019_builder_fit_real_top1"]} the same recipe reached on the same rows '
                     f'in 0018 when it had been fitted on the builder\'s generated families. '
                     f'<strong>And a depth-3 decision tree fitted on the same rows reads {_ba["floor_real_top1"]}.</strong> '
                     f'The clause is the floor those trivial baselines reach plus {_ba["bar"]}, never chance '
                     f'(<span class="mono">docs/OPERATING_RULES.md</span> §4a): {_ba["min_correct_real_over_floor"]} rows '
                     f'were needed and {rfv["ext_real_correct_model"]} arrived, a margin of '
                     f'{round(rfv["ext_real_top1_model"]-_ba["floor_real_top1"],4)} over a rule a person could write out '
                     f'by hand. Under the chance-based bar its predecessor drafted '
                     f'({_ba["min_correct_real_chance_based"]} rows) this would have published as a pass. '
                     f'{_ba["families_clearing"]} of 5 real families clear their own floor plus {_ba["bar"]}; every one is '
                     f'above its own floor, and py_src misses by 0.0033. <strong>Content, not budget</strong>: the same '
                     f'recipe on the same number of the builder\'s rows reads {_rt["builder_matched"]}, and on the same '
                     f'number of the builder\'s plaintexts {_rt["builder_chunk_matched"]}. 0003\'s incumbent on the real '
                     f'rows reads {_rt["real_incumbent"]}, 0014\'s standardised logistic {_rt["real_logistic"]}. The '
                     f'reproduction rung reads {rfv["reproduction_top1"]} against 0003\'s banked '
                     f'{rfv["reproduction_reference"]} and the null control {rfv["shuffled_label_accuracy_ext"]}. '
                     f'<strong>This is an in-distribution reading on real content, not a transfer reading</strong>: the '
                     f'fit and the scored chunks are cut from the same five files and share no byte. Nothing here revises '
                     f'0003, 0014, 0015, 0016, 0017, 0018 or 0019, and nothing here establishes a buyer.</p>'
...
<div class="provenance">chain head {prereg["chain"][-1]["hash"][:16]} · {len(prereg["chain"])} preregistrations, hash-chained, beacon-anchored · rendered at {git_head} from artifacts whose combined sha256 begins {artifacts_sha} · this page is regenerated by tools/render/evidence_page.py and gated by claimcheck</div>
